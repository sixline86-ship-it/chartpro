...
# 🔴🔴 2026-09-02 **같은 사고 재발** — 어제(9/1) 고쳤는데 오늘 또 이 줄이
#    "20260901"로 하드코딩된 채 올라가 발행이 멈췄다. 어제와 똑같은
#    경로: 미리보기 만들 때 사본(/home/claude/preview*)에서 날짜를
#    고정한 뒤, 그 사본을 repo(전달 원본) 위에 통째로 복사해서
#    임시값이 딸려 들어왔다.
#    ⚠️⚠️ 앞으로 절대 지킬 것:
#      1) 미리보기는 **반드시 별도 폴더 사본에서만** 날짜를 고정한다.
#      2) 그 사본을 **절대 repo로 되돌려 복사하지 않는다**.
#      3) 파일을 전달하기 직전에 항상 `grep '^DATE = ' build_html.py`로
#         이 줄이 datetime.now()인지 눈으로 확인한다.
#    [증상] 이 줄이 과거 날짜면, 그날 리포트 HTML을 못 찾아
#    `cp: cannot stat 'report_YYYYMMDD.html'`로 발행이 통째로 실패한다.
# ── 파일 보관 위치 ──
...
def _mkt_supply3(수급):
    ...
    # 🔴 HO 지시 2026-09-22 — 옆 칸(최근 5일 실탄)과 «줄을 맞춘다».
    #   [전] 「순매수 (조 원)」 한 줄 + 숫자 → 칸 가운데 정렬이라 제목이
    #     옆 칸 제목보다 아래에 떴다.
    #   [후] 옆 칸과 똑같이 «제목 / 설명 / 내용» 세 층으로 쌓고, 두 칸 모두
    #     위에서부터 채운다(CSS). 그러면 제목끼리·설명끼리·내용끼리 같은 줄.
    return (f'<p class="sc2-spark-t sc3-t">순매수'
            f'<span class="sc2-spark-def">(단위: 조)</span></p>'
            f'<div class="sc3 sc3-body">{"".join(칸)}</div>')
...
def one_sector_card(a):
    rows = []
    for s in a.get("종목", [])[:4]:
        # 🔴 2026-09-15 — 같은 이유(등락률이 문자열 아닌 숫자로 올 수 있음)로
        #   크래시는 안 나지만 "29.98"처럼 %·+ 없이 밋밋하게 나올 뻔했다.
        _raw = s.get("등락률")
        rate = (f"{_raw:+.2f}%" if isinstance(_raw, (int, float))
                else str(_raw) if _raw is not None else "—")
...
# ── 핵심 이슈 ──
# ══════════════════════════════════════════════════════════════
# 📰 시황 탭 — 4단 재구성 (HO 지시 2026-09-22)
#
#   [무엇이 문제였나] 같은 네 주제(반도체·산업·정책·수급)를 같은 기사로,
#     말투만 바꿔 두 번 썼다(「무슨 일이」 → 「이슈 해부」). 「이슈 해부」는
#     네 개 전부 «왜 하필 오늘 ~했을까요?»로 시작했다 — 템플릿이 보이면
#     독자는 읽지 않고 넘긴다. 투자에 쓸 «내일 볼 것»은 맨 끝 3막에 묻혔다.
#   [새 순서] 사실 → 돈 → 행동
#     ① 📌 오늘 한 줄          — 헤드라인 + 숫자 셋
#     ② 📰 오늘 무슨 일        — 카드 하나에 «사실» + ▾펼침(왜·어디까지)
#     ③ 🔥 오늘 테마는 이렇게  — 들어온/나간 테마 + 해석 한 단락
#     ④ ✅ 내일 확인할 것      — 기준 + «왜 그 기준인가» + 맞으면/아니면
#     ⑤ 🔍 남들이 놓친 자리    — 접어 둔다(원하는 사람만)
#   ⚠️ 새 칸(시황한줄·테마시황·내일확인)이 없는 옛 해석글에서도 깨지지
#      않게, 없으면 옛 칸이나 데이터로 대신 채운다.
def _sh_num(v):
    try:
...
def _sh_headline(data, 해석):
    """① 오늘 한눈에 — 헤드라인 2~3개(HO 지시 2026-09-22).

    [왜 여러 개] 한 줄로는 «지수» 얘기만 남는다. 오늘 시장은 최소 세 겹이다 —
      지수가 어떻게 움직였나 / 누가 사고 팔았나 / 돈이 어디로 갔나.
      세 줄을 위아래로 쌓으면 스크롤 없이 하루가 보인다.
    ⚠️ 옛 해석글(시황한줄이 dict 하나)도 그대로 읽는다.
    """
    h = 해석.get("시황한줄")
    줄 = []
    if isinstance(h, list):
        줄 = [x for x in h if isinstance(x, dict) and x.get("제목")][:3]
    elif isinstance(h, dict) and h.get("제목"):
        줄 = [h]
    if not 줄:
        t = 해석.get("사건명") or 해석.get("오늘의_한문장") or ""
        if not t:
            return ""
        줄 = [{"제목": t, "부제": ""}]
    _ic = ["📈", "💰", "🔄"]
    몸 = "".join(
        f'<div class="sh-hl h{i}{" first" if i == 0 else ""}"><span class="sh-hi">{_ic[i % 3]}</span>'
        f'<div class="sh-hb"><p class="sh-t">{x.get("제목","")}</p>'
        + (f'<p class="sh-s">{x.get("부제","")}</p>' if x.get("부제") else "")
        + '</div></div>' for i, x in enumerate(줄))
...
def _sh_issues(해석):
    ...
    if not its:
        return ('<div class="sh-box"><p class="sh-h">📰 오늘 무슨 일</p>'
                '<p class="sh-empty">오늘 이슈 정리가 아직 준비되지 않았어요.</p></div>')
    카드 = []
    for it in its:
        태그 = it.get("태그", "")
        내용 = it.get("내용", "")
...
def _sh_tomorrow(해석):
    ...
    if not xs:
        ...
        if not 대체:
            return ""
        return (f'<div class="sh-box"><p class="sh-h">✅ 내일 확인할 것</p>'
                f'<p class="sh-p">{대체}</p></div>')
    카드 = []
    for i, x in enumerate(xs, 1):
        카드.append(
            f'<div class="sh-k"><p class="sh-kq"><b>{i}</b>'
...
def build_sihwang(data, 해석):
    return (_sh_headline(data, 해석) + _sh_issues(해석) + _sh_theme(해석)
            + _sh_tomorrow(해석) + _sh_pro(해석))
...
def accum_badge(종목명):
    ...
    # 🔴 HO 지시 2026-09-12 — 「차수 판정 N일치」 배지를 뺀다.
    #    [무엇이었나] 몇 차 매집인지 판정할 때 «되돌아본 거래일 수»다.
    #    목표는 ACC_이력_창(60일)인데 기록이 덜 쌓였을 때만 떴다.
    #    [왜 뺐나] 화면에서 이걸 알아야 할 사람은 없다. 카드가 복잡해질 뿐이다.
    #    → 로직 문서(logic_v17.md 5장)에 남기고 화면에서는 뺀다.
    return 배지
...
def _문턱(유형, cfg):
    """설정에서 금액 문턱(%)을 꺼낸다. 없으면 물음표."""
    _m = (cfg or {}).get("금액문턱") or {}
    v = _m.get(유형)
...
def build_accumulation(매집, 설정=None, 종목사전=None):
    ...
    조건 = dev_note(
        ...
        #   전엔 「며칠 샀나」만 적혀 있었다. 금액도 반대편도 연속성도
        #   안 보고 있다는 사실이 화면에 없었다.
        + (f'<br><b style="color:#5a4208">🔒 2026-09-19 추가된 3중 조건</b><br>'
           f'① <b>금액 문턱</b> — 5일 누적 순매수가 그 회사 시가총액의 '
           f'{_문턱("쌍끌이", cfg)}%(쌍끌이) / {_문턱("기관 단독", cfg)}%(기관 단독) / '
           f'{_문턱("외국인 단독", cfg)}%(외국인 단독) 이상. '
           f'<i>시총 3조 회사에 2억 사는 건 매집이 아니라 우연이라서요. '
           f'실측 205건에서 시총대비 0.5% 아래는 전부 마이너스였고, '
           f'0.5% 위에서 승률이 58%로 뒤집혔습니다.</i><br>'
           f'② <b>반대편이 안 팔 것</b> — 한쪽이 사는데 다른 쪽이 그 '
           f'{int((cfg.get("반대편한도") or 0.5)*100)}%를 넘게 팔면 제외. '
           f'<i>외국인 +100억인데 기관 −60억이면 매집이 아니라 손바뀜이에요.</i><br>'
           f'③ <b>연속성</b> — 최장 <b>{cfg.get("최소연속","?")}일 연속</b> 순매수. '
           f'<i>「+ + − + +」와 「− + + + +」는 같은 4일이지만, 앞은 우연이고 '
           f'뒤는 의지입니다.</i>'
           if cfg.get("금액문턱") else "")
    ) if cfg else ""
...
(function(){
 ...
 function drawBrief(my){
  ...
  my.forEach(function(nm){
   ...
   /* 🔴 HO 지시 2026-09-12 — 토론방 옆에 «차트» 알약.
      [왜] 브리핑을 보다가 «그래서 지금 차트가 어떻지»가 바로 이어진다.
           리포트가 못 그리는 것(실시간 일봉)은 있는 곳으로 보내주면 된다.
      [어디로] 네이버 종목 상세(item/main) — 열면 일봉 차트가 바로 보인다.
   ...
   hits=_hits2;
   n1=hits.length;
   /* 🔴 HO 지시 2026-09-12 — 기사를 2건만 펼치고 나머지는 «더보기»로 접는다.
      [왜] 종목 하나에 기사가 5건씩 붙으면 종목 10개 = 50줄이 된다.
      브리핑이 «훑는» 물건인데 스크롤만 길어졌다. 2건이면 흐름을 잡기에 충분하고,
      더 보고 싶은 사람만 펼치면 된다. */
   var _NW_OPEN=2;
   var _nwid='nw'+(window._NW_SEQ=(window._NW_SEQ||0)+1);
   hits.slice(0,_NW_OPEN).forEach(function(h){
    var n=h.n;
   ...
   /* 나머지 기사 — 접어두고 버튼으로 펼친다 */
   if(n1>_NW_OPEN){
    var rest='';
    hits.slice(_NW_OPEN).forEach(function(h){
     var n=h.n;
     var 날=n.k?'<span style="font-size:9.5px;color:#e0c060;font-weight:700">'+
       (n.o?'오늘':n.k)+'</span> ':'';
     rest+='<div style="display:flex;gap:6px;margin-top:4px"><span style="flex:none">📰</span>'+
      '<div>'+날+'<a href="'+n.u+'" target="_blank" style="font-size:12px;color:#8fb4ee;'+
      'line-height:1.5;text-decoration:none">'+n.t+'</a>'+
      (h.w===1?'<span style="font-size:9.5px;color:#6f7784"> · 본문 언급</span>':'')+
      (n.s?'<span style="font-size:9.5px;color:#6f7784"> · '+n.s+'</span>':'')+
    ...
    items+='<div id="'+_nwid+'" style="display:none">'+rest+'</div>'+
     '<p style="margin:5px 0 0"><span onclick="ztog(&quot;'+_nwid+'&quot;)" '+
     'style="font-size:10px;color:#e0c060;font-weight:700;cursor:pointer">'+
     '▾ 기사 '+(n1-_NW_OPEN)+'건 더보기</span>'+
     '<span style="font-size:9.5px;color:#6f7784"> · 최근 '+(window.CP_BRIEF_DAYS||20)+'거래일</span></p>';
   }
   ...
   /* 🔴 HO 지시 2026-09-12 — 「기업분석」 «바로 옆»에 차트 알약.
      [어디로] 네이버 종목 상세(item/main) — 열면 일봉이 바로 보인다.
   ...
   out+='<div style="padding:11px 0;border-bottom:1px solid #1b212c">'+
    ...
    /* 🆕 2026-09-04 (2차) HO 지시 — 기업분석 펼침칸을 «기사·공시(items)» **위**로
       ...
       아래로 건너뛰었다. 이름 → 기업분석 → 기사/공시 → 오늘 분석 순서로
 ...
 function _fin(nm, 상세){
  ...
  /* 🆕 2026-09-04 HO 지시 — "매출·영업이익·순이익이 2025만 나오는데
     최근 5년 기록이냐"는 질문. [사실] 맞다 — ys 전체(대개 5개년)로 막대
     추이(_bars)를 그리고, 숫자·라벨만 최신연도 것이다. 그런데 기간표시엔
     최신연도만 있어 막대가 몇 년치인지 안 보였다. 연도가 2개 이상이면
     "시작년~끝년 추이"를 앞에 붙여 막대그래프가 몇 년 치인지 바로 보이게 한다. */
  var 기간표시=(ys.length>=2?ys[0]+'~'+ys[ys.length-1]+'년 추이 · ':'')+
   ys[ys.length-1]+'년 '+(끝.구분||'')+' 기준';
 ...
 function scToggle(nm, id){
  ...
  if(!box.dataset.built){
   ...
   /* 🆕 2026-09-04 (3차) HO 지시 — "자세한 내용은~" 안내문이 두 줄(설명+매출)
      로 나뉘어 있고, 매출 없을 때 DART 원문·준비중·지어내지 않는다는 설명이
      너무 길다. [WHY] 앞면은 어디까지나 요약 — 안내는 "더 볼 게 있다"만
      짧게 알리면 되고, 왜 없는지의 사정은 «자세히 보기» 안에서 설명해도
      충분하다. 두 안내를 하나로 합치고, 없을 때는 원칙14대로 조용히 비운다. */
   var _자세히더=(_설명더있음||_bp);
   if(_본문||테마.length||_연혁){
    회사='<div class="sc-blk"><p class="sc-h">🏢 어떤 회사냐면요</p>'+
     ...
     (_자세히더?'<p class="sc-src">사업 부문별 자세한 내용과 매출은 «자세히 보기» 클릭.</p>':'')+'</div>';
   }
 ...
 if(document.readyState!=='loading'){render()}else{
  document.addEventListener('DOMContentLoaded',render)}
    ...
    return ('<div style="background:#141922;border:1px solid #232a36;border-radius:12px;'
            ...
            # 🔴 HO 지시 2026-09-12 — 순서를 뒤집는다.
            #    [기존] 등록하기 → 성적(그래프) → 목록
            #    [변경] 성적(그래프) → 등록하기 → 목록
            #    [왜] 이미 등록한 사람(=매일 보는 사람)에게 등록 폼이 먼저 나오면
            #    매번 스크롤해서 지나쳐야 한다. 성적이 먼저 보여야 «오늘 어땠나»가
            #    바로 읽힌다. 등록 폼은 사실상 처음 한 번만 쓰는 자리다.
            ...
            # 🔴 HO 지시 2026-09-12 — 「내 종목 평균(동일 비중)」 그래프를
            #    기간 탭 «바로 밑»으로 올린다.
            #    [왜] 탭을 눌러 기간을 바꾸는 이유가 이 그래프를 보려는 것인데,
            #    등록 폼·목록·백업 버튼을 다 지나야 나왔다.
            #    ⚠️ id는 문서에 «하나»여야 한다 — 아래에서 지우고 여기로 «옮겼다».
            #       (복제하면 getElementById가 위쪽 빈 div를 잡아 전부 멈춘다)
            '<div id="ms-sum"></div><div id="ms-chart"></div>'
            '<p style="margin:16px 0 6px;font-size:17px;font-weight:800;color:#f0c65a;padding-top:13px;border-top:1px solid #232a36">'
            '📋 내 종목 추적하기</p>'
            ...
            # 🔴 HO 지시 2026-09-12 — 「📖 내 종목 보는 방법」 접힘 블록 삭제.
            #    [왜] 화면이 이미 스스로 설명한다 — %p·섹터 태그·평균 모두
            #    그 자리에 짧은 설명이 붙어 있다. 접힌 채로 아무도 안 여는
            #    도움말이 카드 맨 아래를 차지하고 있었다.
            '</div>' + JS
            + '<script>' + 이름배열JS + """
/* 자체 자동완성 (datalist 대체)
...
# 🔴🔴 2026-09-17 버그 수정 — «누르면 종목이 나온다»가 첫 클릭에 안 먹었다.
#   [원인] 패널의 초기 숨김은 «CSS 클래스»가 한다(.tm-pan{display:none}).
#     그런데 이 함수는 «인라인 스타일»(e.style.display)만 봤다.
#     처음엔 인라인이 비어 있어서(""), "" === "none"이 거짓 →
#     «none»을 새로 써넣는다 → 화면은 그대로. 두 번 눌러야 열렸다.
#   [증상이 안 보인 이유] 에러가 안 난다. 한 번 눌러 보고 "안 되네" 하고
#     넘어가지, 두 번 누를 생각은 안 한다. 어제 발행본에서도 같았다.
#   [고침] «지금 실제로 보이는가»(getComputedStyle)를 기준으로 삼는다.
#   ⚠️ 이 함수는 섹터 지도·섹터 성적표·돌아올 섹터·테마 3코너가 전부 쓴다.
#     여기 한 줄 고치면 그 전부가 같이 낫는다.
ZTOG_JS = ('<script>function ztog(id){var e=document.getElementById(id);'
           'if(!e)return;var v=getComputedStyle(e).display;'
           'e.style.display=(v==="none"?"block":"none");}</script>')
...
def build_core(핵심편, data, 해석):
    ...
    # 🔴 2026-09-22 — 시황 탭 4단 재구성(build_sihwang).
    #   옛 조각(움직인것들·뒤집블록·build_issues·build_insight)은 지우지 않는다
    #   (원칙3). 되돌리려면 아래 한 줄을 옛 조립으로 바꾸면 된다.
    _시황 = build_sihwang(data, 해석) + build_odd_today()
    ...
    _수급 = ((f'<div class="mny">{_mny속}</div>' if _mny속.strip() else '')
             + 격자블록
             # 🔴 2026-09-22 — 수급 탭 5단(build_flow_v2). 옛 관제신호는 맨 아래 접어 둔다.
             + build_flow_v2(data, 해석)
             + (f'<details class="fv-old"><summary>📦 이전 수급 화면 (게이지·비차익 판독·사후 통계) — 정리 중</summary>'
    ...
    _테마 = (_테마앞
             ...
             # ⚠️ 2026-09-17 HO 지시 — 섹터 성적표는 «숨긴다».
             #   [왜] 이건 테마가 아니라 섹터 이야기다. 테마탭 흐름
             #   (지금 어디 → 다음 → 왜 → 오늘 → 맞았나)의 밖에 있었다.
             #   [지우지 않는 이유] 계산은 그대로 돌고 값도 쌓인다.
    ...
    # 🔒 2026-09-17 HO 지시 — 테마탭을 «읽기 / 판단» 두 갈래로 나눈다.
    #   [읽기] 지금까지의 6코너. 데이터 종류로 나뉜 «읽는» 화면.
    #   [판단] 그 6코너를 «교차»해서만 나오는 결론. 유료 전용 후보.
    ...
    if _판단:
        _테마 = (
            '<div class="jd-tabs">'
            '<button class="jd-tb-b on" onclick="jdTab(0,this)">읽기</button>'
            '<button class="jd-tb-b" onclick="jdTab(1,this)">판단 🔒</button>'
            '</div>'
            f'<div id="jdPane0">{_테마}</div>'
            f'<div id="jdPane1" style="display:none">{_판단}</div>'
            + JUDGE_JS)

    # 🔴 v17 (2026-09-12) — 종목 탭 재구성.
    #    [무엇이 문제였나] v16 개편 때 build_radar()·build_accumulation()
    #    (심층편의 «종목 목록» 상세표)이 호출부에서 통째로 빠졌다.
    #    함수는 살아 있는데 아무 데서도 부르지 않아 화면에 안 나왔다.
    #    → 핵심편 요약(build_core_*) 아래에 대장주, 그 아래 상세표를 둔다.
    #    구성: 요약(무엇이 잡혔나) → 대장주(그래서 뭘 보나) → 상세(전체 목록)
    ...
    # 🔴 HO 지시 2026-09-12 — 「내 종목」을 별도 탭으로 분리한다.
    #    [왜] 「포착」은 우리가 찾아낸 것, 「내 종목」은 구독자가 등록한 것.
    #    성격이 다른데 한 탭에 섞여 있어 스크롤이 길었다.
    _내종목 = (build_my_stocks(data)
               + build_stock_brief())
    ...
    _탭목록 = [("종합", "오늘 시장 한눈에", _종합),
             ...
             #    hide()를 벗기면 그동안의 기록이 그대로 살아난다.
             ("성적표", "우리 예보는 맞았나", hide("성적표탭", _성적표))]
...
def _note_md(txt):
    """짧은 마크다운 → HTML.
    «### 제목» 소제목 · «- 항목» 목록 · «**굵게**» · 빈 줄 = 문단
    🆕 2026-09-24 (가독성) — HO «글자가 너무 많다, 들어온 무리·나가는 무리처럼»
      «> 글»           = 한 줄 포인트 상자
      «@ 제목 | 이름 | 설명» = 무리 카드 (연속이면 격자로 나란히)
      «! 글»           = 주의 한 줄
    """
    import re as _re, html as _h
    # 🆕 2026-09-24 HO — «빨강은 실제 빨강 글씨, 주황은 실제 주황 글씨로(글자만)»
    #   «{빨강:글}» → 레이더 점과 «같은 색» 글자. 색 이름이 곧 색이 되게 한다.
    _색 = {"노랑": TM_NEW, "빨강": TM_HOT, "주황": TM_WARM,
          "초록": TM_COOL, "파랑": TM_DOWN}
    def _inl(t):
        t = _h.escape(t)
        t = _re.sub(r"\*\*(.+?)\*\*", r"<b>\1</b>", t)
        return _re.sub(r"\{(노랑|빨강|주황|초록|파랑):(.+?)\}",
                       lambda m: f'<b style="color:{_색[m.group(1)]}">{m.group(2)}</b>', t)
    out, ul, cards = [], [], []
    def _flush():
        if ul:
            ...
            ul.clear()
        if cards:
            out.append('<div class="cn-cards">' + "".join(cards) + "</div>")
            cards.clear()
    for blk in str(txt).split("\n"):
        ...
        if st.startswith("@ "):
            ...
            p = [x.strip() for x in st[2:].split("|")] + ["", ""]
            cards.append(f'<div class="cn-card"><p class="cn-ct">{_inl(p[0])}</p>'
                         f'<p class="cn-cn">{_inl(p[1])}</p>'
                         f'<p class="cn-cd">{_inl(p[2])}</p></div>')
            continue
        _flush()
        if st.startswith("### "):
            out.append(f"<h5>{_inl(st[4:])}</h5>")
        elif st.startswith("> "):
            out.append(f'<div class="cn-pt">{_inl(st[2:])}</div>')
        elif st.startswith("! "):
            out.append(f'<p class="cn-warn">❗ {_inl(st[2:])}</p>')
        elif st.startswith("- "):
            ul.append(_inl(st[2:]))
        else:
            out.append(f"<p>{_inl(st)}</p>")
    _flush()
    return "".join(out)
...
def _chapter_note(key, guides=""):
    ...
    if not v:
        return ""
    # 🔴 2026-09-24 HO — «읽는 방법 / 해석과 판단 두 칸. 읽는 방법은 따로 화살표로
    #   접어줘 — 매번 뜨는 게 피로하다.»
    # 🔴 2026-09-24 (2차) HO — «읽는 방법이 동떨어져 있어 위 챕터인지 아래 챕터인지
    #   헷갈린다. 챕터 맨 아래 ⚠️ 안내문 자리에 두고, 그 안내문도 읽는 방법에 넣어라.»
    #   → 순서: [챕터 본문] → 📖 읽는 방법(노트 + 챕터의 ⚠️·📌 안내문) → 💡 해석과 판단
    #     guides = _chapter_block()이 챕터에서 뽑아 온 안내문 HTML.
    if isinstance(v, dict):
        해석 = _note_md(v.get("해석") or "")
        읽는법 = _note_md(v.get("읽는법") or "")
    else:
        해석, 읽는법 = _note_md(v), ""
    읽는법 = 읽는법 + (f'<div class="cn-guide">{guides}</div>' if guides else "")
    html = ""
    if 읽는법:
...
def _pull_guides(html, classes):
    """챕터 HTML에서 «읽는 법» 성격의 안내 요소(클래스로 지정)를 떼어 낸다.
    반환: (안내문 뺀 본문, 떼어 낸 안내문 HTML). div는 중첩을 세어 끝을 찾는다."""
    import re as _re
    뽑음 = []
    for cls in classes:
        while True:
            m = _re.search(r'<(p|div)\s+class="' + _re.escape(cls) + r'(?:\s[^"]*)?"[^>]*>', html)
            if not m:
                break
            tag, st = m.group(1), m.start()
            if tag == "p":
                e = html.find("</p>", m.end())
                if e < 0:
                    break
                ...
                while depth and k < len(html):
                    a = html.find("<div", k); b = html.find("</div>", k)
                    if b < 0:
                        break
                    if 0 <= a < b:
                        depth += 1; k = a + 4
...
#   [왜] 코너마다 색이 다른 걸 가리키면 구독자가 매번 범례를 다시 읽어야 한다.
#   색조는 원래도 거의 같았지만 주황이 #ffa63c / #ff9838로 미세하게 달랐다.
#   이제 한 곳에서만 정의한다 — 바꿀 일이 생기면 여기만 고친다.
TM_HOT  = "#ff5a4e"   # 가장 셈    — 1~5위 / 고속 / 올라오는 중
TM_WARM = "#ff9838"   # 중간       — 6~10위 / 가속
TM_COOL = "#3ecf9a"   # 약함·안정  — 11~20위 / 서행 / 제자리
TM_DOWN = "#5b9bff"   # 빠짐       — 밀려남 / 후진 (리포트 전체 «하락=파랑» 규칙)
TM_NEW  = "#ffc93c"   # 신규       — 오늘 첫 등장 (유일하게 노랑을 쓰는 자리)
TM_FLAT = "#8a93a2"   # 정지·무판정

# 테마 순위 → 색 (3단. 4단은 주황↔노랑 ΔE 28로 구별 안 됐다)
def _tcol(rk):
    return TM_HOT if rk <= 5 else (TM_WARM if rk <= 10 else TM_COOL)
...
# ══════════════════════════════════════════════════════════════
# 🆕 2026-09-17 — 테마 «숲» 보기용 계산기 묶음.
#
#   [왜 만드나] 지금 테마탭은 테마를 «한 개씩» 보여준다. 10개를 나란히
#   늘어놓지만, 그 10개가 «한 덩어리인지 흩어져 있는지»는 말해주지 않는다.
#   실측: 2026-09-17 10위권 중 «7개가 반도체»였다. 그건 「테마 10개가 떴다」가
#   아니라 「반도체 하나가 7갈래로 보이는 것」이다. 투자 판단이 정반대로
#   갈리는 차이인데(분산인 줄 알고 집중하게 된다) 화면엔 없었다.
#
#   ⚠️ 새로 긁는 데이터가 없다. 전부 theme_history.json에 이미 있다.
#      네이버가 또 뭘 없애도 이 코너들은 안 죽는다.
def _theme_zone_map():
    """테마명 → 섹터(구역). 최근 기록이 이긴다."""
    일별 = _theme_hist_all()
    out = {}
    for dd in sorted(일별):                     # 오름차순 → 뒤가 덮어쓴다
        for x in (일별.get(dd) or []):
            nm, z = x.get("테마명"), x.get("구역")
            if nm and z:
                out[nm] = z
    return out
...
def _theme_amt_map():
    ...
    for dd in sorted(일별):
        for x in (일별.get(dd) or []):
            nm, a = x.get("테마명"), x.get("거래대금")
            if nm and a:
                out.setdefault(nm, {})[dd] = a
    return out


def market_breadth():
    """오늘 «시장의 폭» — 10위권이 몇 개 섹터에 흩어져 있나."""
    rk = _theme_cum_rank()
    days = sorted(rk)
    if not days:
        return None
    ...
    def 한날(d):
        top = [n for n, _ in (rk.get(d) or [])[:10]]
        cnt = {}
        for n in top:
            z = zm.get(n)
            if z:
        ...
        if not cnt:
            return None
        best = max(cnt.items(), key=lambda v: v[1])
        return {"섹터수": len(cnt), "최대섹터": best[0], "최대개수": best[1],
                "분포": sorted(cnt.items(), key=lambda v: -v[1])}

    오늘 = 한날(days[-1])
    if not 오늘:
        return None
    추이 = [(d, r["섹터수"], r["최대개수"])
           for d in days[-6:] for r in [한날(d)] if r]
    # ⚠️ «좁다/넓다»는 남과 비교해야 뜻이 산다. 저장된 날 중 몇 번째로
    #    좁은지를 같이 낸다 — 숫자 하나만 던지면 읽는 사람이 기준이 없다.
    전체 = [x for x in (한날(d) for d in days) if x]
    오늘.update({"추이": 추이, "표본일수": len(전체),
               "더좁은날": sum(1 for x in 전체 if x["섹터수"] < 오늘["섹터수"])})
    return 오늘
...
def build_theme_survival(오늘나이=None):
    ...
    if not s:
        return ""
    mx = max(v for _n, v in s["곡선"]) or 100
    막대 = "".join(
...
def theme_board():
    """🧮 테마 «종합판» — 6코너가 따로 들고 있던 값을 한 줄로 모은다.

    [왜 만드나] 테마 하나에 대해 «순위·어제·나이·돈·섹터»가 각기 다른
    코너에 흩어져 있었다. 독자는 여섯 군데를 오가며 머릿속에서 합쳐야
    했다. 합치는 건 리포트가 할 일이다.

    [진짜 값어치는 «교차»에 있다] 각 값은 혼자선 이미 아는 얘기다.
      · 순위 1위            → 레이더가 말한다
      · 4일째               → 나이가 말한다
      · 절반 빠지는 건 3일째 → 생존곡선이 말한다
    ...
    if not days:
        return [], {}
    오늘d = days[-1]
    어제d = days[-2] if len(days) >= 2 else None
    오늘 = {nm: i + 1 for i, (nm, _s) in enumerate(rk.get(오늘d) or [])}
    어제 = ({nm: i + 1 for i, (nm, _s) in enumerate(rk.get(어제d) or [])}
           if 어제d else {})
    # 5일 전 자리 — 레이더와 «같은 창»을 쓴다(어긋나면 두 화면이 다른 말을 한다)
    _win = days[-5:]
    과거, 과거일 = {}, {}
    for _k, dd in enumerate(_win[:-1]):
        for i, (nm, _s) in enumerate(rk.get(dd) or []):
            if nm not in 과거:
                과거[nm] = i + 1
                과거일[nm] = len(_win) - 1 - _k      # 🔴 2026-09-24 — «며칠 전»을 실제로 센다
    나이맵, 중앙, 표본 = _theme_age_map()
    zm, am = _theme_zone_map(), _theme_amt_map()

    out = []
    for nm, pos in sorted(오늘.items(), key=lambda v: v[1])[:10]:
        a = (am.get(nm) or {}).get(어제d) if 어제d else None
        b = (am.get(nm) or {}).get(오늘d)
        # ⚠️ 한쪽이라도 없으면 «0%»가 아니라 None이다. 모르면 침묵한다.
        돈 = ((b / a - 1) * 100) if (a and b) else None
        f5 = 과거.get(nm)
        out.append({"n": nm, "rk": pos, "y": 어제.get(nm), "from5": f5,
                    "from5_ago": 과거일.get(nm),
                    "st": move_tier(f5, pos), "age": 나이맵.get(nm),
                    "돈": 돈, "zone": zm.get(nm) or "기타"})
    생존 = theme_survival() or {}
    메타 = {"중앙": 중앙, "표본": 표본, "생존중앙": 생존.get("중앙값"),
           "생존표본": 생존.get("완결수"), "어제있음": bool(어제d)}
    return out, 메타
...
JUDGE_JS = ('<script>'
            'function jdTab(i,el){'
            ' for(var k=0;k<2;k++){var p=document.getElementById("jdPane"+k);'
            '  if(p)p.style.display=(k===i?"block":"none");}'
            ' var bs=el.parentElement.querySelectorAll(".jd-tb-b");'
            ' for(var j=0;j<bs.length;j++)bs[j].className='
            '  "jd-tb-b"+(j===i?" on":"");'
            # ⚠️ 첫 그리기에 키를 넘기면 «같은 키 재클릭»으로 읽혀
            #    방향이 뒤집힌다(10위 → 1위로 나왔다). 인자 없이 부른다.
            ' if(i===1)jdDraw();}'
            'var JD_SORT={k:"rk",asc:true};'
            'function jdDraw(k){'
            ' var rows=(window.JD_ROWS||[]).slice();'
            ' if(k){ if(JD_SORT.k===k){JD_SORT.asc=!JD_SORT.asc;}'
            '        else{JD_SORT.k=k;JD_SORT.asc=(k==="rk"||k==="age");} }'
            ' var key=JD_SORT.k, asc=JD_SORT.asc;'
            ' rows.sort(function(a,b){'
            # ⚠️ 값이 없는 칸(null)은 «0»이 아니라 «맨 뒤»다.
            #    0으로 두면 「돈 0%」인 척하게 되어 거짓말이 된다.
            '  var x=a[key],y=b[key];'
            ...
            '  var m=(r.m===null||r.m===undefined)?"–":'
            '        (r.m>0?"+"+r.m+"%":r.m+"%");'
            '  var mc=(r.m===null||r.m===undefined)?"#5f6875":'
            '         (r.m>=15?"#74f0d4":(r.m<=-15?"#e8c33a":"#7d8695"));'
            '  var y=(r.y===null||r.y===undefined)?"밖":r.y+"위";'
            '  h+="<div class=\'jd-rw\'><span class=\'jd-nm\' style=\'color:"+r.c+"\'>"'
            '    +r.n+"</span><span>"+r.rk+"위</span><span>"+y+"</span><span>"'
            '    +((r.age===null||r.age===undefined)?"–":r.age+"일")'
            # ⚠️ 섹터명이 길면(에너지·정유·화학) 테마명을 밀어낸다.
            #    격자의 주인공은 테마명이다 — 섹터는 앞 4글자로 줄인다.
...
def _judge_light(t, 메타):
    """교차 판정 — 색, 이름, 근거. 판단이 아니라 «조건 대조»다."""
    생존중앙 = 메타.get("생존중앙")
    근거 = []
    # ── 판단 보류: 잴 자가 없으면 재지 않는다 ──
    if t["from5"] is None or t["age"] is None:
        return ("gray", "판단 보류", ["5일 전 기록이 모자라 견줄 자리가 없어요"])
    늦음 = (생존중앙 is not None and t["age"] > 생존중앙)
    내림 = t["st"] == "down"
    돈빠짐 = (t["돈"] is not None and t["돈"] <= -15)
    돈늘음 = (t["돈"] is not None and t["돈"] >= 15)

    if 내림 or 돈빠짐:
        if 내림:
            근거.append(f'순위가 {t.get("from5_ago") or 4}일 전 {t["from5"]}위 → {t["rk"]}위로 밀렸어요')
        if 돈빠짐:
            근거.append(f'거래대금이 어제보다 {t["돈"]:.0f}% 줄었어요')
        return ("red", "식는 중", 근거)

    if 늦음:
        근거.append(f'{t["age"]}일째 — 절반이 빠지는 지점({생존중앙}일)을 넘겼어요')
        if t["rk"] <= 3:
            근거.append(f'자리는 {t["rk"]}위로 좋지만 시간이 늦은 편이에요')
        return ("yellow", "조건은 맞는데 늦음", 근거)

    if t["st"] in ("hot", "warm", "new"):
        근거.append(f'{t.get("from5_ago") or 4}일 전 {t["from5"]}위 → {t["rk"]}위로 올라오는 중')
    ...
    return ("gray", "판단 보류", ["올라오지도 밀리지도 않는 자리예요"])


def _judge_conflicts(rows, 메타):
    """⚡ 엇갈리는 신호 — 두 값이 «다른 말»을 하는 자리만 모은다.

    [왜 이게 핵심인가] 보통 요약은 모순을 뭉갠다. 여러 지표를 하나로
    눌러 «종합 87점»을 만든다. 그러면 뭐가 좋고 뭐가 나쁜지가 사라진다.
    투자 판단은 대부분 «엇갈리는 지점»에서 일어난다 — 그 지점을
    숨기지 말고 그대로 보여준다.
...
def build_judge_tab(data=None):
    ...
    if b:
        ...
        한줄 = (f'<b>{b["최대섹터"]}</b> 한 곳으로 좁아진 장입니다. '
               ...
               if 좁 else
               f'<b>{b["섹터수"]}개 섹터</b>로 흩어진 장입니다. '
               f'한 곳에 오래 머무르기보다 갈아타기가 맞는 자리예요.')
        받침 = (f'10위권 중 {b["최대섹터"]} {b["최대개수"]}개'
               + (f' · 최근 {b["표본일수"]}거래일 중 '
                  + ('가장 좁음' if b["더좁은날"] == 0
                     else f'{b["더좁은날"] + 1}번째로 좁음')
                  if b.get("표본일수", 0) >= 5 else ""))
        층1 = (f'<div class="jd-1"><p class="jd-1h">📐 오늘 한 줄</p>'
               f'<p class="jd-1t">{한줄}</p>'
               f'<p class="jd-1s">{받침}</p></div>')
    else:
        층1 = ""
    ...
    층2 = (f'<div class="jd-2"><p class="jd-h">🚦 오늘 테마 상태'
           ...
           + (f'(생존 표본 {메타["생존표본"]}건).' if 메타.get("생존표본") else '.')
           + f'</p></div>')

    # ── 3층: 한눈 격자 (정렬 가능) ────────────────────
    import json as _json
    _dat = [{"n": t["n"], "rk": t["rk"], "y": t["y"], "age": t["age"],
             "m": (round(t["돈"]) if t["돈"] is not None else None),
             "z": t["zone"],
             "c": _MC[t["st"]]} for t in rows]
    층3 = (f'<div class="jd-3"><p class="jd-h">📋 한눈 격자'
           ...
           f'<div id="jdBody"></div></div>'
           f'<p class="jd-note">같은 데이터인데 <b>내가 오늘 중요하게 보는 '
           f'기준</b>으로 다시 세울 수 있어요. 「나이」를 누르면 어린 테마부터, '
           f'「돈」을 누르면 돈이 늘어난 순입니다.</p></div>'
           f'<script>window.JD_ROWS={_json.dumps(_dat, ensure_ascii=False)};</script>')
...
#   [무엇을 보나] 날짜마다 막대 하나 = 그날 돈 전체(100%). 띠 두께 = 섹터 몫.
#     시장 전체가 커지고 작아지는 «밀물·썰물»을 빼고, «몫의 이동»만 남긴다.
#   ⚠️ 시안의 재료는 아직 옛 방식(10위권 테마 합 — 중복 포함)이다.
#      실제 적용은 중복 없는 «섹터거래대금»이 5거래일 쌓인 뒤.
def _mf_v2(dsw, 순):
    ...
    for i, d in enumerate(dsw):
        tot = sum((v["per"].get(d, 0) or 0) for _z, v in 순)
        x = left + i * cw + (cw - bw) / 2
        if tot <= 0:
            sv.append(f'<rect x="{x:.1f}" y="{top}" width="{bw:.1f}" height="{H-top-bot}" '
                      f'fill="url(#mfhatch)" stroke="#2a3548"/>')
            sv.append(f'<text x="{x+bw/2:.1f}" y="{top-8}" fill="#5f6b7d" font-size="8" '
                      f'text-anchor="middle">기록없음</text>')
        else:
            y = H - bot
            for k, (z, v) in enumerate(순):
                a = v["per"].get(d, 0) or 0
                if a <= 0:
                    continue
                h = (H - top - bot) * a / tot
                y -= h
                c = MF_PAL[min(k, len(MF_PAL) - 1)]
                sv.append(f'<rect x="{x:.1f}" y="{y:.1f}" width="{bw:.1f}" height="{h:.1f}" '
                          f'fill="{c}" opacity="0.9"/>')
                if h >= 13 and i == n - 1:
                    sv.append(f'<text x="{x+bw/2:.1f}" y="{y+h/2:.1f}" fill="#0a0d12" '
                              f'font-size="8.5" font-weight="800" text-anchor="middle" '
                              f'dominant-baseline="central">{a/tot*100:.0f}%</text>')
            조 = tot / 1_000_000
            sv.append(f'<text x="{x+bw/2:.1f}" y="{top-8}" fill="#8b95a5" font-size="8" '
                      f'text-anchor="middle">{조:.0f}조</text>')
        if i % 3 == 0 or i == n - 1:
            sv.append(f'<text x="{x+bw/2:.1f}" y="{H-bot+13}" fill="#6f7a8b" font-size="8.5" '
                      f'text-anchor="middle">{d[4:6]}/{d[6:8]}</text>')
    범례 = "".join(f'<span style="display:inline-flex;align-items:center;gap:4px;margin:0 9px 4px 0">'
                 f'<i style="width:9px;height:9px;border-radius:2px;background:'
                 f'{MF_PAL[min(k, len(MF_PAL)-1)]}"></i>{z}</span>'
                 for k, (z, _v) in enumerate(순))
    return ('<div style="margin:14px 0 6px;padding:11px 10px 8px;border:1px dashed #3b4a63;'
            ...
            '<defs><pattern id="mfhatch" width="6" height="6" patternUnits="userSpaceOnUse" '
            'patternTransform="rotate(45)"><line x1="0" y1="0" x2="0" y2="6" '
            'stroke="#3a4558" stroke-width="2"/></pattern></defs>'
            + "".join(sv) + '</svg>'
            f'<div style="font-size:10.5px;color:#aab4c2;margin-top:6px">{범례}</div></div>')
...
def build_money_flow(win=10):
    ...
    return (f'<div class="mf-box"><p class="mf-h">🌊 돈의 이동 경로'
            ...
            f'<span class="mf-td"></span></div></div>{흐름}{_mf_v2(dsw, 순)}'
            # 🔴 2026-09-24 HO — «읽는 방법을 따로 접어줘. 항시 나오게 하지 말고.»
            #   📈 결론 한 줄(위 흐름)은 그날의 정보라 그대로 두고,
            #   매일 같은 📌 안내만 «📖 읽는 방법»으로 접는다(레이더와 같은 모양).
            f'<details class="cn-how" style="margin:8px 0 2px"><summary>📖 읽는 방법</summary>'
            f'<div class="cn-hb">'
            f'<p class="mf-f" style="margin:0">📌 <b>막대가 3일 연속 커지면</b> 돈이 들어오는 중 — '
            ...
            f'(2026-09-10까지 하루 10개만 저장).</p></div></details></div>')
...
def build_market_breadth():
    """📐 오늘 시장의 폭 — 테마탭 맨 위 «판단» 카드.

    [왜 맨 위인가] 나머지 코너는 전부 «관찰»(뭐가 떴다·다음은 뭐다)이다.
    독자는 여섯 코너를 다 읽고 «스스로» 종합해야 했다. 종합은 리포트가
    할 일이다. 그 한 줄을 먼저 말하고, 아래 코너들이 그 근거가 된다.

    [무엇을 재나] 10위권 테마가 «몇 개 섹터»에 흩어져 있나.
      · 흩어짐(7종) = 순환매 장. 여기저기 기회. 분산이 먹힌다.
      · 쏠림(3종)   = 한 곳에서만 골라야 한다. 밖에 있으면 소외되고,
                      그 한 곳이 꺾이면 10위권 대부분이 같이 꺾인다.
    ⚠️ 쏠림은 좋은 것도 나쁜 것도 아니다. «대응이 달라지는» 신호다.
       좋다/나쁘다로 쓰지 않는다.
    """
    b = market_breadth()
    if not b:
        return ""
    섹터수, 최대, 개수 = b["섹터수"], b["최대섹터"], b["최대개수"]
    # 폭 등급 — 섹터 종류로 가른다(10칸을 몇 갈래로 나눠 가졌나).
    if 섹터수 <= 3:
        등급, 색, 말 = "매우 좁음", TM_HOT, "한 곳으로 몰렸습니다"
    elif 섹터수 <= 4:
        등급, 색, 말 = "좁음", TM_WARM, "몇 곳에 모였습니다"
    elif 섹터수 <= 6:
        등급, 색, 말 = "보통", TM_COOL, "적당히 흩어져 있습니다"
    else:
        등급, 색, 말 = "넓음", TM_DOWN, "여러 곳에 흩어져 있습니다"
    # 막대 — 섹터 8종을 만점으로 «폭»을 그린다(좁을수록 왼쪽).
    w = max(6, min(100, round(섹터수 / 8 * 100)))

    # 추이 — 어제·그제와 견줘 «좁아지는 중인가»를 말한다. 이게 핵심이다.
    추 = b.get("추이") or []
    흐름 = ""
    if len(추) >= 3:
        최근 = [x[1] for x in 추[-3:]]
        if 최근[0] > 최근[1] > 최근[2]:
            흐름 = (f'<b style="color:{TM_HOT}">3거래일 연속 좁아지는 중</b>'
                   f'(섹터 {최근[0]}종 → {최근[1]}종 → {최근[2]}종)')
        elif 최근[0] < 최근[1] < 최근[2]:
            흐름 = (f'<b style="color:{TM_DOWN}">3거래일 연속 넓어지는 중</b>'
                   f'(섹터 {최근[0]}종 → {최근[1]}종 → {최근[2]}종)')
        else:
            흐름 = f'최근 3거래일 섹터 종류 {최근[0]} → {최근[1]} → {최근[2]}종'

    # 자리매김 — 저장된 날 중 몇 번째로 좁은가
    순위말 = ""
    if b.get("표본일수", 0) >= 5:
        더좁 = b.get("더좁은날", 0)
        순위말 = (f'최근 {b["표본일수"]}거래일 중 '
                 + (f'<b>가장 좁습니다</b>' if 더좁 == 0
                    else f'<b>{더좁 + 1}번째로 좁습니다</b>'))

    # 분포 칩
    칩 = "".join(
        f'<span class="mb-z{" on" if z == 최대 else ""}">{z} <b>{c}</b></span>'
        for z, c in (b.get("분포") or [])[:5])

    # 무엇을 해야 하나 — «판단»은 여기서만 한다.
    if 섹터수 <= 4:
        행동 = (f'<li><b>{최대}</b> 밖에 있으면 오늘은 소외됩니다 '
               f'— 고를 자리가 그 안에 있습니다.</li>'
               f'<li><b>{최대}</b>가 꺾이면 10위권 <b>{개수}개</b>가 '
               f'같이 꺾입니다 — 여러 테마를 담아도 <b>분산이 아닙니다</b>.</li>')
    else:
        행동 = ('<li>여러 곳에서 돌아가며 뜨는 장입니다 — '
               '한 테마에 오래 머무르기보다 <b>갈아타기</b>가 맞는 자리예요.</li>'
               '<li>한 곳이 꺾여도 다른 곳이 받쳐 줍니다 '
               '— <b>분산이 실제로 분산</b>이 됩니다.</li>')

    return (f'<div class="mb-card">'
            f'<p class="mb-h">📐 오늘 시장의 폭'
            f'<span style="color:{색}">{등급}</span></p>'
            f'<p class="mb-lead">10위권 테마가 <b>{섹터수}개 섹터</b>에 나뉘어 '
            f'있습니다 — {말}. 가장 많은 곳은 <b style="color:{색}">{최대} '
            f'{개수}개</b>예요.</p>'
            f'<div class="mb-bar"><i style="width:{w}%;background:{색}"></i></div>'
            f'<p class="mb-ax"><span>좁음</span><span>넓음</span></p>'
            f'<div class="mb-zs">{칩}</div>'
            + (f'<p class="mb-tr">{흐름}'
               + (f' · {순위말}' if 순위말 else "") + '</p>' if 흐름 else "")
            + f'<ul class="mb-act">{행동}</ul>'
            f'<p class="mb-note">⚠️ 쏠림은 좋은 것도 나쁜 것도 아닙니다. '
...
def _theme_members(data):
    ...
    """
    out = {}
    # 🔴 2026-09-15 — 9/15 밤 실제 발행에서 AttributeError로 리포트 생성이
    #   통째로 멈췄다. 원인: collect_data.py의 새 목록-API 경로가 「등락률」을
    #   **숫자**(29.98)로 채우기 시작했는데, 여기(그리고 그 아래 _stock_panel)는
    #   옛 HTML 스크래핑 시절 그대로 **"+1.43%" 형태의 문자열**을 기대하고
    #   .startswith("+")를 호출하고 있었다 — 숫자엔 그 메서드가 없어 죽었다.
    #   collect_data.py 쪽 숫자 타입은 그대로 둔다(그쪽은 정렬·비교에 숫자가
    #   필요하다) — 대신 화면에 넘기기 직전인 여기서 문자열로 통일한다.
    def _등락문자(v):
        if v is None:
            return None
        if isinstance(v, str):
            return v          # 이미 문자열(옛 경로) — 그대로 둔다
        try:
            return f"{float(v):+.2f}%"
        except (TypeError, ValueError):
            return None
...
def _stock_panel(title, items, pid):
    ...
    🆕 2026-09-17 ⑤ — 종목에 «역할»을 붙인다.
      [왜] 지금은 등락률 순으로 늘어놓기만 했다. 그런데 투자 판단에
      필요한 건 «순서»가 아니라 «자리»다.
        · 대장   = 오늘 그 테마를 끌고 간 종목. 테마가 살았나 죽었나를
                   여기서 본다. 대장이 꺾이면 테마가 꺾인 것이다.
        · 후발   = 대장을 따라가는 중인 종목. 갭 메우기 후보.
        · 소외   = 테마인데 안 오른 종목. «아직 안 온 것»일 수도,
                   «테마와 사실상 무관한 것»일 수도 있다 — 둘 다 가능성으로 둔다.
...
def move_key(작게=False):
    ...
    return ('<div class="tm-key' + (' tm-key-s' if 작게 else '') + '">'
            + "".join(f'<span><i style="background:{_MC[k]}'
                      + (';box-shadow:0 0 0 1.5px #0d1119,0 0 0 2.6px #ffffff'
                         if k == "new" else "")
                      + f'"></i>{t}</span>'
                      for k, t in 항목) + '</div>')
...
def build_theme_radar(data):
    ...
    for r in rows:
        # 🔴 2026-09-24 HO — «"5일 전 3위"면 4일 전이라고 해야 하지 않아?»
        #   맞다. 창은 오늘 포함 5거래일이라 가장 이른 날은 «4거래일 전»이다.
        #   게다가 그날 20위 밖이었으면 시작점은 더 늦은 날(3·2·1일 전)이 된다.
        #   → 고정 문구 «5일 전» 대신, 시작점이 실제로 «며칠 전»인지 센다.
        _idx = [k for k, x in enumerate(r["h"][:-1]) if x and x <= THEME_TOPN]
        r["from"] = r["h"][_idx[0]] if _idx else None
        r["from_ago"] = (len(r["h"]) - 1 - _idx[0]) if _idx else None
    ...
    for r in rows:
        ...
        # ── 꼬리 = «5거래일 창의 첫 기록» 자리 → 오늘 자리 ──
        #   ⚠️ 어제 대비가 아니다. 오늘 포함 최근 5거래일 중 가장 이른 날(4거래일 전)의
        #      순위에서 출발한다. 그날 20위 밖이었으면 창 안에서 처음 20위 안에 든 날.
        #      어제 대비는 아래 목록의 «어제 N위 ▲▼»가 맡는다.
        # 🔴 HO 지시 2026-09-24 — «첫 등장도 꼬리가 있는 게 좋지 않나? 10위권 밖에서
        #   올라왔으니 시작점 없는 점선으로. 원 밖으로 나오게 그려도 되고.»
        #   [전] 11~20위에서 온 테마는 시작점이 «10위 고리 위»에 찍혀 «10위에서 왔다»로
        #        잘못 읽혔고, 첫 등장(창 안에서 20위 밖)은 꼬리가 아예 없었다.
        #   [후] 원 밖 = 10위 밖 — 한 가지 시각 규칙으로 통일.
        #        · 1~10위에서 옴  : 원 안 시작점 ○ + 점선 (기존 그대로)
        #        · 11~20위에서 옴 : 원 «밖»에 시작점 ○ (순위만큼 바깥으로) + 점선
        #        · 첫 등장         : 원 밖 멀리서 들어오는 점선, 시작점 없음
        #                           («어디서 왔는지 모른다»는 사실 그대로)
        # 🔴 2026-09-24 HO — «기판·전력반도체·S7이 다 같은 시작점인데?»
        #   [원인] 셋은 실제로 12~13위로 거의 같았고, 간격도 1칸=1.7px라
        #     눈으로 구별이 안 됐다. → 간격을 1칸=2.6px로 넓히고,
        #     원 밖 시작점 옆에 «13» 같은 순위 숫자를 작게 붙인다.
        #     그림이 같아 보여도 숫자가 «어디서 왔는지»를 정확히 말한다.
        def _out_r(rank):                        # 11위 → 고리 바로 밖, 20위 → +29px
            return RR(10) + 6 + (min(rank, 20) - 11) * 2.6
        if r["from"] is not None and r["from"] <= 10:
            ox, oy = POS(r["from"], a); 시작점 = True
        elif r["from"] is not None:
            _t = math.radians(a - 90); _rr = _out_r(r["from"])
            ox, oy = CX + _rr * math.cos(_t), CY + _rr * math.sin(_t); 시작점 = True
        else:
            _t = math.radians(a - 90); _rr = RR(10) + 36
            ox, oy = CX + _rr * math.cos(_t), CY + _rr * math.sin(_t); 시작점 = False
        d = math.hypot(x - ox, y - oy)
        if d > 1:
            ux, uy = (ox - x) / d, (oy - y) / d
            sv.append(f'<line x1="{x+ux*(rad+2):.1f}" y1="{y+uy*(rad+2):.1f}" '
                      f'x2="{ox:.1f}" y2="{oy:.1f}" stroke="{c}" stroke-width="1.5" '
                      f'stroke-dasharray="{"3 3" if 시작점 else "2 4"}" '
                      f'opacity="{0.65 if 시작점 else 0.75}"/>')
            if 시작점:
                sv.append(f'<circle cx="{ox:.1f}" cy="{oy:.1f}" r="2.6" fill="none" '
                          f'stroke="{c}" stroke-width="1.2" opacity="0.7"/>')
                if r["from"] > 10:               # 원 밖 시작점엔 출발 순위 숫자
                    #   섹터 이름 글자와 겹치면 옆으로 비켜 앉는다(9/23 시안: 전자파 «14»가
                    #   «기타 1» 위에 겹침)
                    _lr = _out_r(r["from"]) + 9
                    for _da in (0, 9, -9, 16, -16):
                        _tt = math.radians(a + _da - 90)
                        _lx, _ly = CX + _lr*math.cos(_tt), CY + _lr*math.sin(_tt)
                        _bx = (_lx - 7, _ly - 6, 14, 12)
                        if not any(_bx[0] < p[0] + p[2] and p[0] < _bx[0] + _bx[2]
                                   and _bx[1] < p[1] + p[3] and p[1] < _bx[1] + _bx[3]
                                   for p in placed):
                            break
                    sv.append(f'<text x="{_lx:.1f}" '
                              f'y="{_ly:.1f}" fill="{c}" '
                              f'font-size="8" font-weight="700" opacity="0.9" '
                              f'text-anchor="middle" dominant-baseline="central">'
                              f'{r["from"]}</text>')
        sv.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{rad+4:.1f}" fill="{c}" opacity="0.16"/>')
        # 🔴 2026-09-24 HO — «주황과 노랑이 구분이 잘 안 된다.»
        #   색은 리포트 전체가 같은 상수(TM_NEW 등)를 쓰므로 여기서만 바꾸면 규칙이 깨진다.
        #   대신 «첫 등장»에만 흰 점선 고리를 두른다 — 색이 헷갈려도 모양으로 구별된다.
        if r["st"] == "new":
            sv.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{rad+3.2:.1f}" fill="none" '
    ...
    #     사라졌으면 그게 오늘의 헤드라인인데, 지금은 각주로 밑에 있었다.
    #   [왜 돈인가] 순위가 오르는 길은 둘이다 — «내 돈이 늘어서» 오른 것과
    #     «남들이 더 빠져서» 상대적으로 오른 것. 앞은 진짜, 뒤는 가짜다.
    #     실측(9/17): 스페이스X 15→5위 대금 +92%(진짜) /
    #                 SOFC 3→7위 대금 −44%(식는 중).
    #     지금 리포트는 이 둘을 구분 못 했다 — «들어가도 되나»에 직접
    #     답하는 값인데도.
    _어제순위 = {}
    if len(days) >= 2:
        _어제순위 = {nm: i + 1 for i, (nm, _s) in enumerate(rk.get(days[-2]) or [])}
    _대금 = _theme_amt_map()
    ...
    def _money_badge(nm):
        ...
        if not _어제d:
            return ""
        a = (_대금.get(nm) or {}).get(_어제d)
        b = (_대금.get(nm) or {}).get(_오늘d)
        if not a or not b:
            return ""          # ⚠️ 없는 날은 «0%»가 아니라 침묵이다
        ch = (b / a - 1) * 100
        if ch >= 15:
            return (f'<span class="tm-mn up">💰 돈도 늘었어요 '
                    f'+{ch:.0f}%</span>')
        if ch <= -15:
            return (f'<span class="tm-mn dn">⚠️ 순위만 올랐어요 '
                    f'(돈 {ch:.0f}%)</span>')
        return f'<span class="tm-mn fl">돈은 비슷 {ch:+.0f}%</span>'
    ...
    for r in rows:
        ...
        mv = (f"{len(r['h']) - 1}일간 20위 밖 → 첫 등장" if r["from"] is None
              else f"{r['from_ago']}일 전 {r['from']}위 → {r['h'][-1]}위")
        # 어제 대비 — 새로 들어왔나, 몇 칸 움직였나
        _y = _어제순위.get(r["n"])
        if _y is None:
    ...
    # 🆕 2026-09-17 ② «테마 뭉치» — 갈라진 것을 다시 묶어 보여준다.
    #   [왜] 화면에선 온디바이스 AI(1위)와 HBM(3위)이 «별개 테마»로 보인다.
    #     독자는 "둘 다 사야 하나?" 생각한다. 그런데 사실은 «같은 돈이 두
    #     이름으로 잡힌 것»이다. 둘 다 사면 분산이 아니라 «집중»이다.
    #     이걸 안 보여주면 «분산한 줄 알고 집중하는» 사고가 난다.
    #   ⚠️ 순위 번호는 그대로 둔다. 묶는 건 «읽는 순서»를 바꾸는 게 아니라
    ...
    _순 = sorted(_그룹.items(), key=lambda kv: (kv[0] == "기타", -len(kv[1])))
    _묶음 = []
    for _z, _items in _순:
        _n = len(_items)
        _c = TM_HOT if _n >= 4 else (TM_WARM if _n >= 2 else TM_FLAT)
        _묶음.append(
            f'<div class="tm-cl"><p class="tm-clh" style="color:{_c};'
            f'border-color:{_c}44">{_z}<b>{_n}개</b>'
            # ⚠️ 여기에 «%»를 쓰면 위 「시장의 폭」 카드(10위권 기준)와
            #    숫자가 어긋난다. 이 화면은 8개만 그리기 때문이다.
            #    같은 화면에서 두 숫자가 다르면 둘 다 못 믿게 된다.
            #    → 분모를 «말로» 밝힌다.
...
/* ══════ 🆕 2026-09-17 테마 «숲» 코너들 ══════ */
/* 📐 오늘 시장의 폭 — 탭 맨 위 «판단» 카드.
   ⚠️ 아래 코너들보다 «한 단계 위»로 보여야 한다. 테두리를 밝게 주고
      배경을 살짝 띄운다 — 이게 결론이고 아래는 근거라는 표시다. */
.mb-card{background:linear-gradient(160deg,#141c26,#101620);
  border:1.5px solid #2c3d50;border-radius:13px;padding:14px 14px 12px;
  margin:0 0 12px}
.mb-h{margin:0 0 7px;font-size:14.5px;font-weight:800;color:#e8ecf1;
  display:flex;align-items:baseline;gap:7px}
.mb-h span{font-size:12px;font-weight:800}
.mb-lead{margin:0 0 9px;font-size:12.5px;color:#c3cad4;line-height:1.75}
.mb-lead b{color:#e8ecf1}
.mb-bar{height:7px;border-radius:4px;background:#1a2330;overflow:hidden}
.mb-bar i{display:block;height:100%;border-radius:4px}
.mb-ax{display:flex;justify-content:space-between;margin:3px 1px 8px;
  font-size:9px;font-weight:700;color:#5f6875}
.mb-zs{display:flex;flex-wrap:wrap;gap:4px;margin-bottom:8px}
.mb-zs span{font-size:9.5px;font-weight:700;color:#8b93a0;background:#151d27;
  border:1px solid #232f3d;border-radius:999px;padding:2px 7px}
.mb-zs span.on{color:#e8ecf1;border-color:#3d5568;background:#1b2734}
.mb-zs b{font-weight:800}
.mb-tr{margin:0 0 9px;font-size:10.5px;color:#8b93a0;line-height:1.7}
.mb-act{margin:0;padding:9px 0 0 17px;border-top:1px solid #1e2937;
  font-size:11.5px;color:#aeb6c1;line-height:1.75}
.mb-act li{margin-bottom:3px}
.mb-act b{color:#e0e5ec}
.mb-note{margin:7px 0 0;font-size:10px;color:#6f7784;line-height:1.6}
/* 🧩 테마 뭉치 — 같은 섹터끼리 묶은 덩어리 */
.tm-cl{margin-bottom:6px}
.tm-clh{display:flex;align-items:center;gap:6px;margin:10px 0 0;
...
.tm-mn{font-size:9.5px;font-weight:800;border-radius:999px;padding:1.5px 7px;
  white-space:nowrap}
.tm-mn.up{color:#74f0d4;background:rgba(116,240,212,.11)}
.tm-mn.dn{color:#e8c33a;background:rgba(232,195,58,.11)}
.tm-mn.fl{color:#7d8695;background:rgba(125,134,149,.1)}
/* 종목 역할 — 대장 / 후발 / 소외 */
.tm-chip em{font-style:normal;font-size:8.5px;font-weight:800;
  border-radius:3px;padding:0 4px;margin:0 4px 0 3px;vertical-align:1px}
.r-lead{color:#0b0e13;background:#ffc93c}
.r-mid{color:#ffc93c;border:1px solid rgba(255,201,60,.45)}
...
.cn-b b{color:#fff}
.cn-pt{margin:2px 0 10px;padding:9px 11px;border-radius:9px;background:#172233;
  border-left:3px solid #e8c33a;font-size:13px;font-weight:700;color:#f1f4f8;line-height:1.6}
.cn-cards{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin:6px 0 10px}
@media (max-width:340px){.cn-cards{grid-template-columns:1fr}}
.cn-card{padding:8px 9px;border-radius:9px;background:#121b28;border:1px solid #223047;min-width:0}
.cn-ct{margin:0 0 3px!important;font-size:11px;font-weight:800;color:#8fc1ff}
.cn-cn{margin:0 0 3px!important;font-size:12px;font-weight:800;color:#fff;line-height:1.45;
  word-break:keep-all}
.cn-cd{margin:0!important;font-size:11px;color:#9aa6b6;line-height:1.5}
.cn-warn{font-size:11.5px;color:#c9a34a}
.cn-how{margin:-6px 0 14px}
.cn-how summary{cursor:pointer;list-style:none;display:inline-flex;align-items:center;gap:5px;
  padding:4px 2px;font-size:11.5px;font-weight:700;color:#7d8aa0}
.cn-how summary::-webkit-details-marker{display:none}
.cn-how summary::before{content:"▸";font-size:10px;transition:transform .15s}
.cn-how[open] summary::before{transform:rotate(90deg)}
.cn-hb{padding:8px 12px 10px;border-radius:9px;background:#0d141e;border:1px solid #1d2838;
  font-size:12px;line-height:1.75;color:#b3bdca}
.cn-guide{margin-top:8px;padding-top:8px;border-top:1px dashed #243044}
...
.jd-tb-b.on{color:#0b0e13;background:#8fd0e8;border-color:#8fd0e8}
.jd-wrap{display:block}
...
.jd-1{background:linear-gradient(160deg,#16202c,#111823);
  ...
  margin-bottom:11px}
.jd-1h{margin:0 0 6px;font-size:10.5px;font-weight:800;color:#8fd0e8}
.jd-1t{margin:0 0 6px;font-size:14px;font-weight:700;color:#eef1f5;
  line-height:1.65}
.jd-1t b{color:#fff}
.jd-1s{margin:0;font-size:10.5px;color:#7d8695;line-height:1.6}
/* 2층 — 신호등 */
.jd-2{background:#101720;border:1px solid #1e2937;border-radius:12px;
  padding:12px 13px 11px;margin-bottom:11px}
.jd-grp{margin-bottom:9px}
.jd-gh{display:flex;align-items:center;gap:6px;margin:0 0 5px;padding:0 0 4px;
  border-bottom:1px solid;font-size:11.5px;font-weight:800}
.jd-gh span{margin-left:auto;font-size:10px;opacity:.8}
.jd-it{display:flex;align-items:center;gap:7px;padding:7px 2px;
  font-size:12.5px;color:#dfe4ea;cursor:pointer;
  border-bottom:1px solid #161d26}
.jd-it b{font-weight:700}
.jd-rk{font-size:10px;font-weight:800;min-width:30px}
.jd-why{display:none;padding:6px 2px 8px 37px;font-size:11px;color:#8b93a0;
  line-height:1.75}
.jd-why p{margin:0}
.jd-warn{margin:8px 0 0;padding-top:8px;border-top:1px solid #1b2530;
  font-size:10px;color:#7d8695;line-height:1.7}
.jd-warn b{color:#b6bfcb}
/* 3층 — 한눈 격자 */
.jd-3{background:#101720;border:1px solid #1e2937;border-radius:12px;
  padding:12px 13px 11px;margin-bottom:11px}
.jd-tb{font-size:10.5px}
/* ⚠️ 헤더・행의 가로 칸을 하나로 봇는다. 
   테마명 칸에 최소폭을 주지 않으면 숫자 칸들이 이름을 밀어내
   「양자암호/…」처럼 세 글자만 남는다 — 주인공은 이름이다. */
.jd-hd,.jd-rw{display:grid;
  grid-template-columns:minmax(0,1fr) 30px 32px 28px 40px 42px;
  gap:4px;align-items:center}
.jd-hd{padding:0 0 6px;border-bottom:1px solid #223041;
  font-size:9.5px;font-weight:800;color:#6f7784}
.jd-hd span{text-align:right;cursor:pointer;user-select:none}
.jd-hd span:first-child{text-align:left}
.jd-hd span:hover{color:#8fd0e8}
.jd-rw{padding:7px 0;border-bottom:1px solid #161d26}
.jd-rw span{text-align:right;font-weight:700;color:#9aa3b1;
  font-variant-numeric:tabular-nums}
.jd-nm{text-align:left !important;font-weight:700;overflow:hidden;
  text-overflow:ellipsis;white-space:nowrap}
.jd-z{font-size:9px !important;font-weight:600 !important;color:#6f7784 !important;
  overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.jd-note{margin:8px 0 0;font-size:10px;color:#6f7784;line-height:1.7}
.jd-note b{color:#8b93a0}
/* 4층 — 엇갈리는 신호 */
.jd-4{background:#101720;border:1px solid #1e2937;border-radius:12px;
  padding:12px 13px 11px}
.jd-cf{padding:9px 0;border-bottom:1px solid #161d26}
.jd-cn{margin:0 0 3px;font-size:12.5px;font-weight:800;color:#e2e7ee}
.jd-cv{margin:0 0 3px;font-size:11px;color:#9aa3b1;line-height:1.6}
...
.sv-h{margin:0 0 6px;font-size:12.5px;font-weight:800;color:#e2e7ee;
  display:flex;align-items:baseline;gap:7px}
...
.sv-b{flex:1;height:9px;background:#161e29;border-radius:3px;overflow:hidden}
.sv-b i{display:block;height:100%;background:#3d6b8a;border-radius:3px}
.sv-v{width:34px;flex:none;text-align:right;font-size:10px;font-weight:800;
  color:#9aa3b1;font-variant-numeric:tabular-nums}
/* «오늘 이 테마가 있는 자리» — 곡선 위 내 위치 */
.sv-r.me .sv-n,.sv-r.me .sv-v{color:#ffc93c}
.sv-r.me .sv-b i{background:#ffc93c}
.sv-note{margin:9px 0 0;padding-top:8px;border-top:1px solid #1b2530;
  font-size:10px;color:#6f7784;line-height:1.7}
...
# ══════════════════════════════════════════════════════════════
# 👑 오늘의 대장주 (v17, 2026-09-12)
#
#   「테마별로 가장 대장다운 종목 1개씩」 — 테마 순위 순으로 최대 6개.
#
#   [강세 레이더와 뭐가 다른가] 실측 겹침 1종목/98종목.
#     강세 레이더 = 시총 5,000억 이상 «큰 회사» 중 오늘 터진 것
#     대장주      = 테마 안에서 가장 센 것 (시총 제한 없음)
#   실제로 대장주 6개 중 5개가 시총 1,000위 밖이다. 강세 레이더는
#   이런 종목을 조건상 애초에 보지 않는다. 겹치지 않는다.
#
#   ⚠️ 「오늘 뜬 테마」는 테마 «안»의 4종목을 보여준다. 대장주는 테마를
#      «가로질러» 뽑는다. 질문이 다르므로 테마당 1개로 고정한다.
# ══════════════════════════════════════════════════════════════
...
def _approx_cap(rank):
    """시총 «금액»이 아직 저장되지 않아 순위로 근사한다.

    ⚠️ 임시다. collect_data가 종목사전에 시총을 담기 시작하면
       이 함수를 지우고 실제 금액을 쓴다(그날부터 회전율이 정확해진다).
    """
    import math
    try:
        return max(200.0, 5_000_000 * math.exp(-float(rank) / 380.0))
    except Exception:
        return 100_000.0


def _leader_stay(name, days=3):
    """최근 days거래일 중 이 종목이 테마 상위4에 이름을 올린 횟수."""
    일별 = _theme_hist_all()
    if not 일별:
        return 1
    cnt = 0
    for d in sorted(일별)[-days:]:
        for x in (일별.get(d) or []):
            if name in (x.get("종목들") or []):
                cnt += 1
                break
    return max(cnt, 1)
...
def build_theme_leaders(data):
    ...
    for ti, s in enumerate(S, 1):
        ...
        for x in (s.get("종목") or []):
            ...
            대금 = x.get("거래대금") or 0
            try:
                등 = float(_re.sub(r"[^0-9.\-+]", "", str(x.get("등락률") or "0")) or 0)
            except Exception:
                등 = 0.0
            v = 사전.get(nm)
    ...
    # 🔴 테마 순서가 아니라 «대장 점수» 순으로 세운다.
    #    [사고] 처음엔 테마 순서대로 넣고 앞에서 잘랐더니 화면이
    #    1위 76점 · 2위 88점처럼 «순위와 점수가 거꾸로» 나왔다.
    out.sort(key=lambda c: -c["sc"])
    out = out[:LEADER_MAX]

    if not out:
        return '<div class="tm-none">오늘 조건을 만족한 대장주가 없습니다.</div>'

    # 🔴 HO 지시 2026-09-12 — 순위(#) 열을 뺀다.
    #    [왜] 이미 점수 내림차순으로 세워져 있어 순서가 곧 순위다.
    #    숫자를 한 번 더 쓰면 폭만 먹고 정보는 늘지 않는다.
    #    대신 그 폭을 «테마명»에 준다 — 어느 테마의 대장인지가 핵심이라서.
    head = ('<div class="ld-h"><span>종목</span>'
            '<span>등락</span><span>회전</span><span>연속</span><span>점수</span></div>')
    rows, tails = [head], []
...
.ld-n{font-size:12px;font-weight:700;min-width:0}
/* 🔴 배지가 아랫줄로 떨어지던 문제.
   [원인] .cp-sname이 <b>라 이름과 배지가 한 덩어리인데, 칸이 좁으면
   그 덩어리 «안»에서 줄바꿈이 일어났다. .sc-tap i의 14px 아이콘이
   줄높이를 밀어 더 도드라졌다.
   [해결] 이름+배지를 통째로 nowrap 인라인블록으로 묶고, 좁을 땐
   이름 쪽이 말줄임되게 한다(배지는 절대 안 밀린다). */
...
.ld-f{margin-top:9px;padding-top:8px;border-top:1px solid #1a212b;
  font-size:9.5px;color:#5a6474;line-height:1.65}

/* 🔴 내 종목 브리핑 이름줄 — «이름 + 등락률 + 기업분석 + 차트»가 한 줄에.
   [실측] 컨테이너 274px인데 네 요소 합이 223px이라 «들어가야» 했는데 두 줄이 됐다.
   원인은 .sc-tap i의 14px 아이콘 + 배지 여백이 실제 폭을 부풀린 것.
   여기서만 배지·등락률을 한 단계 줄인다(다른 코너의 배지는 건드리지 않는다). */
...
# ══════════════════════════════════════════════════════════════
# 💰 수급 탭 5단 재구성 (HO 지시 2026-09-22)
#
#   [무엇이 문제였나]
#     · 같은 숫자(실탄 +1.51조)가 게이지·큰 숫자·막대·「두 가지 확인」에 네 번.
#     · 「누가 샀나」를 셋(개인·외국인·기관)으로만 셌다. 9/21 실측: 셋의 합이
#       −1.66조 — 제로섬이므로 «나머지 주체(기타법인 등)가 +1.66조»를 샀는데,
#       그게 기관(+1.51조)보다 컸다. 오늘의 진짜 매수자가 화면에 없었다.
#     · 타임라인이 말없이 그려져 «무엇을 보라는 그림인지» 알 수 없었다.
#   [새 순서] 누가 넣었나 → 외국인은 어디 서 있나 → 흐름 → 개인 온도 → 믿어도 되나
#   ⚠️ 옛 화면(build_flow_signal)은 지우지 않고 맨 아래에 접어 둔다(원칙3).
def _fv_f(v):
    try:
        return float(str(v).replace(",", ""))
    except (TypeError, ValueError):
...
def _fv_who(data):
    ...
    for 시장 in ("코스피", "코스닥"):
        r = 수.get(f"{시장}_수급") or {}
        개, 외, 기 = _fv_f(r.get("개인")), _fv_f(r.get("외국인")), _fv_f(r.get("기관계"))
        if None in (개, 외, 기):
            continue
        # ⚠️ 제로섬 — 세 주체의 합의 반대 부호가 «그 밖의 주체»(기타법인 등)다.
        #    직접 수집한 값이 아니라 «계산상» 값이므로 화면에 그렇게 적는다.
        그 = -(개 + 외 + 기)
        ...
        #   가능성이 크다. 정체를 확인하기 전까지 «숨은 매수자»로 해석하지 않는다.
        #   ⚠️ 한 번 «진짜 매수자는 기타법인»이라고 말했다가 거둬들였다(교훈).
        # 🔴 HO 지시 2026-09-22 — 순서는 외국인·기관·개인. «그 밖»은 뺀다.
        #   [왜 빼나] 매일 거의 같은 +1.5~1.8조라 «누가 샀나»에 답하지 못한다.
        #     읽는 사람이 판단을 바꿀 정보가 아니면 화면에 두지 않는다.
        줄 = [("외국인", 외, "#f472b6"), ("기관", 기, "#5eead4"), ("개인", 개, "#a78bfa")]
        mx = max(abs(v) for _, v, _ in 줄) or 1
        행 = []
        for 이름, v, c in 줄:
            w = abs(v) / mx * 50
            막 = (f'<span class="fv-bar"><i style="left:{50 if v >= 0 else 50 - w:.1f}%;'
                  f'width:{w:.1f}%;background:{TM_HOT if v >= 0 else TM_DOWN}"></i></span>')
            키 = {"개인": "개인", "외국인": "외국인", "기관": "기관"}.get(이름, "그밖")
            _누 = 누.get(키)
    ...
    btn = "".join(f'<button class="fv-tb{" on" if i == 0 else ""}" '
                  f"onclick=\"fvTab(this,'fvP{i}')\">{m}</button>" for i, (m, _) in enumerate(판))
    pane = "".join(f'<div class="fv-p" id="fvP{i}" style="display:{"block" if i == 0 else "none"}">{b}</div>'
                   for i, (_, b) in enumerate(판))
    return (f'<div class="fv-box"><p class="fv-h">💰 오늘 돈은 누가 넣고 누가 뺐나</p>'
            f'<div class="fv-tabs">{btn}</div>{pane}</div>'
            '<script>function fvTab(el,id){var p=el.parentElement.parentElement;'
            'p.querySelectorAll(".fv-p").forEach(function(x){x.style.display=(x.id===id?"block":"none")});'
...
def _fv_price(이력):
    ...
    for win in (20, 60):
        ...
        if len(rows) < min(win, 10):
            continue
        판 = [(-r["외현"], r["종가"]) for r in rows if r["외현"] < 0]
        산 = [(r["외현"], r["종가"]) for r in rows if r["외현"] > 0]
        if not 판:
            continue
        pa = sum(w * c for w, c in 판) / sum(w for w, _ in 판)
        ba = (sum(w * c for w, c in 산) / sum(w for w, _ in 산)) if 산 else None
        now = rows[-1]["종가"]
        net = sum(r["외현"] for r in rows)
        gap = (now / pa - 1) * 100
        lo = min([pa, now] + ([ba] if ba else [])); hi = max([pa, now] + ([ba] if ba else []))
        span = (hi - lo) or 1
        def X(v):
            return 6 + (v - lo) / span * 88
        점 = (f'<i class="fv-m s" style="left:{X(pa):.1f}%"><em>매도 {pa:,.0f}</em></i>'
              + (f'<i class="fv-m b" style="left:{X(ba):.1f}%"><em>매수 {ba:,.0f}</em></i>' if ba else "")
              + f'<i class="fv-m now" style="left:{X(now):.1f}%"><em>오늘 {now:,.0f}</em></i>')
        if _prev is None:
            말 = (f'외국인은 지금보다 <b>{abs(gap):.1f}% 싼</b> 값에 팔았습니다. 되사려면 판 값보다 '
                  f'비싸게 사야 해서 쉽게 돌아오기 어려운 자리지만, 돌아온다면 «쫓아 사는» '
                  f'급한 매수가 나올 수 있는 자리이기도 해요.' if gap > 0 else
                  f'외국인이 판 값보다 지금이 <b>{abs(gap):.1f}% 쌉니다.</b> 판 값보다 싸게 '
                  f'되살 수 있는 자리라, 되돌아올 여지가 있는 구간이에요.')
        else:
            # ⚠️ 두 번째 창은 같은 문장을 반복하지 않는다 — «길게 보면 무엇이 다른가»만.
            _d = pa - _prev
            말 = (f'더 길게 보면 매도 평균이 <b>{pa:,.0f}</b>로 {"더 낮아요" if _d < 0 else "더 높아요"}'
                  f'({_d:+,.0f}). ' + ('오래전에 판 물량일수록 지금과 거리가 멀어서, '
                  '외국인 입장에선 «되사기 가장 부담스러운» 물량이 뒤에 쌓여 있는 셈이에요.'
                  if _d < 0 else '최근에 판 물량이 오히려 더 싸서, 가까운 매물부터 되살 여지가 있어요.'))
        _prev = pa
        ...
        칸.append(f'<div class="fv-pw fv-pp" data-w="{win}" style="display:{"block" if _show else "none"}">'
                  ...
                  f'<p class="fv-pm">{말}</p></div>')
    if not 칸:
        return ""
    # 🔴 HO 지시 2026-09-22 — 20일·60일을 «탭»으로. 두 판이 위아래로 쌓이면 길다.
    ...
    return (f'<div class="fv-box fv-price"><p class="fv-h">🎯 외국인이 판 가격대'
            ...
            f'<p class="fv-note">※ «매도 평균»은 외국인이 순매도한 날들의 코스피 종가를 '
            f'그날 매도금액만큼 무겁게 쳐서 평균 낸 값, «매수 평균»은 순매수한 날들로 같은 계산을 '
            f'한 값입니다(코스피 현물 기준).</p></div>'
            '<script>function fvW(el,w){var b=el.closest(".fv-price");'
            'b.querySelectorAll(".fv-pp").forEach(function(x){x.style.display=(+x.dataset.w===w?"block":"none")});'
            'b.querySelectorAll(".fv-tb").forEach(function(x){x.className="fv-tb"+(x===el?" on":"")});}</script>')


def _fv_timeline_caption(이력):
    """③ 타임라인 위 «이 그림이 말하는 것» 한 줄."""
    rows = [r for r in 이력[-20:] if r.get("종가")]
    if len(rows) < 5:
        return ""
    ch = (rows[-1]["종가"] / rows[0]["종가"] - 1) * 100
    외 = sum(r.get("외현") or 0 for r in rows)
    기 = sum(r.get("기관") or 0 for r in rows)
    주 = "기관" if 기 > 외 else "외국인"
    return (f'<p class="fv-cap">📌 <b>이 그림이 말하는 것</b> — 최근 {len(rows)}거래일 코스피 '
            f'<b style="color:{TM_HOT if ch>=0 else TM_DOWN}">{ch:+.1f}%</b> · 외국인 '
            f'<b style="color:{TM_HOT if 외>=0 else TM_DOWN}">{_fv_amt(외)}</b> · 기관 '
            f'<b style="color:{TM_HOT if 기>=0 else TM_DOWN}">{_fv_amt(기)}</b>. '
            + (f'지수는 올랐는데 외국인은 팔았어요 — 지수를 받친 건 외국인이 아니었습니다.'
...
def _fv_tl_caption(이력, p):
    """③ 기간 탭마다 «이 그림이 말하는 것» — 같은 문장을 되풀이하지 않는다.

    🔴 HO 지시 2026-09-22 — 탭을 누를 때마다 특징이 달라야 한다.
      [각 탭의 질문]
        5일  = «지금 방향이 바뀌고 있나» — 연속·전환·가장 큰 하루
        20일 = «지수와 돈이 같은 편인가» — 괴리·현물 vs 선물·비차익
        60일 = «큰 흐름과 개인의 빚» — 누적 방향·신용·만기
      ⚠️ 끊긴 데이터(선물·비차익·신용)는 «있는 날만» 쓰고, 그렇다고 밝힌다.
    """
    rows = [r for r in 이력[-p:] if r.get("종가")]
    if len(rows) < 3:
        return ""
    n = len(rows)
    ch = (rows[-1]["종가"] / rows[0]["종가"] - 1) * 100
    외 = [r.get("외현") for r in rows]
    기 = [r.get("기관") for r in rows]
    외s = sum(v for v in 외 if v is not None)
    기s = sum(v for v in 기 if v is not None)
    선 = [r.get("외선") for r in rows if r.get("외선") is not None]
    비 = [r.get("비차익") for r in rows if r.get("비차익") is not None]
    ...
    def streak(vals):
        ...
        if not vs:
            return 0, 0
        sgn = 1 if vs[-1] > 0 else -1
        k = 0
        for v in reversed(vs):
            if v != 0 and (v > 0) == (sgn > 0):
                k += 1
            else:
                break
        return sgn, k
    ...
    if p == 5:
        sg, k = streak(외)
        vs = [v for v in 외 if v is not None]
        prev = vs[:-k] if 0 < k < len(vs) else []
        if k >= 2 and prev:
            말.append(f'외국인이 <b>{k}일 연속 {"순매수" if sg > 0 else "순매도"}</b>입니다 — '
                      f'그 전 {len(prev)}일은 {B(sum(prev))}였어요. '
                      + ('방향이 막 바뀐 자리라, 크기가 붙는지가 관건입니다.'
                         if (sum(prev) > 0) != (sg > 0) else '같은 방향이 이어지는 중입니다.'))
        big = max(((abs(v), i, v) for i, v in enumerate(외) if v is not None), default=None)
        if big:
            d = rows[big[1]]["날짜"]
            kr = rows[big[1]].get("코스피등락") or 0
            말.append(f'5일 중 외국인이 가장 크게 움직인 날은 <b>{d[4:6]}/{d[6:]}</b>({B(big[2])})이고, '
                      f'그날 코스피는 <b style="color:{C(kr)}">{kr:+.2f}%</b>였습니다.')
        sgi, ki = streak(기)
        if ki >= 3:
            말.append(f'기관은 <b>{ki}일 연속 {"순매수" if sgi > 0 else "순매도"}</b>로 '
                      f'{"받치는" if sgi > 0 else "빼는"} 쪽을 꾸준히 지키고 있어요.')
    elif p == 20:
        if ch > 0 and 외s < 0:
            말.append(f'{n}거래일 코스피 <b style="color:{C(ch)}">{ch:+.1f}%</b>인데 외국인은 {B(외s)}, '
                      f'기관은 {B(기s)} — <b>지수를 받친 건 외국인이 아니었습니다.</b>')
        elif ch < 0 and 외s > 0:
            말.append(f'{n}거래일 코스피 <b style="color:{C(ch)}">{ch:+.1f}%</b>인데 외국인은 {B(외s)}를 '
                      f'샀어요 — 외국인이 사는데도 지수가 밀렸다면 다른 쪽 매도가 더 컸다는 뜻입니다.')
        else:
            말.append(f'{n}거래일 코스피 <b style="color:{C(ch)}">{ch:+.1f}%</b> · 외국인 {B(외s)} · '
                      f'기관 {B(기s)} — 지수와 큰돈의 방향이 같습니다.')
        if len(선) >= 3:
            ss = sum(선)
            말.append(f'선물은 외국인 {B(ss)}'
                      + (f'(기록 있는 {len(선)}일만)' if len(선) < n else '') + ' — '
                      + ('현물을 파는 동안 선물은 사 모았어요. 현물 매도를 «선물로 되사 두는» 헤지일 수 있습니다.'
                         if 외s < 0 and ss > 0 else
                         '현물·선물 모두 팔았어요 — 방향을 같이 뺀, 가장 약한 조합입니다.'
                         if 외s < 0 and ss < 0 else
                         '현물·선물 모두 샀어요 — 방향을 같이 건, 가장 강한 조합입니다.'
                         if 외s > 0 and ss > 0 else
                         '현물은 사면서 선물은 팔았어요 — 오른 만큼 선물로 가려 두는 모양입니다.'))
        if len(비) >= 3:
            bs = sum(비)
            말.append(f'프로그램 비차익은 {B(bs)}'
                      + (f'(기록 있는 {len(비)}일만)' if len(비) < n else '')
                      + ' — ' + ('바구니째(여러 종목을 한 번에) 사는 돈이 들어왔어요.' if bs > 0
                                 else '바구니째(여러 종목을 한 번에) 파는 돈이 더 컸어요.'))
    else:
        말.append(f'{n}거래일 동안 외국인 {B(외s)} · 기관 {B(기s)} · 코스피 '
                  f'<b style="color:{C(ch)}">{ch:+.1f}%</b>'
                  + (f' — 기록이 {n}일이라 60일을 다 채우진 못했어요.' if n < 60 else '.'))
        try:
            m = {}
            for _d, dd in archive_days(80):
                c = dd.get("신용잔고") or {}
                for h in (c.get("이력") or []):
                    if isinstance(h.get("잔고"), (int, float)):
                        m[h["날짜"]] = h["잔고"]
                if isinstance(c.get("잔고"), (int, float)) and c.get("기준일"):
                    m[c["기준일"]] = c["잔고"]
            sv = [m[r["날짜"]] for r in rows if r["날짜"] in m]
            if len(sv) >= 5:
                dv = sv[-1] - sv[0]
                말.append(f'개인의 빚(신용잔고)은 이 기간 {B(dv)} — '
                          + ('지수가 오르는 동안 빚도 늘었어요. 개인이 빌려서 따라붙는 중이라, '
                             '꺾이는 날 매물이 한꺼번에 나올 수 있는 자리입니다.' if dv > 0 and ch > 0 else
                             '지수가 빠지는데 빚은 늘었어요 — 물타기 신호일 수 있어 가장 조심할 조합입니다.'
                             if dv > 0 else '빚은 줄었어요 — 개인이 레버리지를 걷어내는 중입니다.'))
        except Exception:
            pass
        만 = [r for r in rows if r.get("만기")]
        if 만:
            d = 만[-1]["날짜"]
            말.append(f'이 구간엔 <b>선물 만기({d[4:6]}/{d[6:]})</b>가 끼어 있어요 — 만기 전후엔 '
                      f'선물·비차익이 포지션 정리로 크게 흔들리니, 그날 숫자는 방향보다 «청산»으로 읽어야 합니다.')
    if not 말:
        return ""
    return ('<div class="fv-cap"><p class="fv-cap-h">📌 이 그림이 말하는 것 '
            f'<span>{p}일</span></p>' + "".join(f'<p>{x}</p>' for x in 말[:3]) + '</div>')
...
def _fv_retail(data):
    ...
    예, 예증 = c.get("예탁금"), c.get("예탁금증감")
    if 신증 is None or 예증 is None:
        return ('<div class="fv-box"><p class="fv-h">🏦 개인 온도<span>예탁금 × 신용</span></p>'
                '<p class="fv-wait">새 경로(증시자금동향)로 바꾼 뒤 <b>첫 수집을 기다리는 중</b>입니다. '
                '들어오면 «개인이 현금으로 들어오나, 빚내서 들어오나»를 네 칸으로 보여드려요.</p></div>')
    칸 = {(True, True): ("🔥 개인 과열", "현금도 늘고 빚도 늘었어요 — 빚까지 내서 들어오는 중"),
         (True, False): ("🙂 조심스러운 유입", "현금은 늘고 빚은 줄었어요 — 무리 없이 들어오는 중"),
         (False, True): ("⚠️ 위험 신호", "현금은 빠지는데 빚은 늘었어요 — 버티려고 빌리는 중일 수 있음"),
         (False, False): ("🧊 개인 이탈", "현금도 빚도 줄었어요 — 개인이 물러나는 중")}
    k = (예증 >= 0, 신증 >= 0)
    제, 설 = 칸[k]
    bd = c.get("기준일") or ""
    return (f'<div class="fv-box"><p class="fv-h">🏦 개인 온도<span>예탁금 × 신용 · '
            f'{bd[4:6]}/{bd[6:]} 기준(2일 늦게 발표)</span></p>'
            f'<p class="fv-rt">{제}</p><p class="fv-rs">{설}</p>'
            f'<div class="fv-rg"><span>고객예탁금 <b>{예/10000:,.1f}조</b> '
            f'<i style="color:{TM_HOT if 예증>=0 else TM_DOWN}">{_fv_amt(예증)}</i></span>'
            f'<span>신용잔고 <b>{신/10000:,.1f}조</b> '
...
.tabsec{{position:relative;border-radius:14px;padding:14px 13px 18px;
  ...
  background:var(--tabbg,transparent)}}
...
/* 🔴 HO 지시 2026-09-12 — 왼쪽 색 띠를 걷어낸다.
   ...
   [대신] 같은 색을 **곳곳에 조금씩** 흘린다:
     · 제목 아래 밑줄 — 색이 «선»이 아니라 «마감»으로 쓰인다
     · 제목 앞 점(::before) — 4px 동그라미 하나
     · 맨 위 아주 옅은 그라데이션 — 스크롤하면 자연히 사라진다
   ⚠️ 어느 것도 글자 대비를 건드리지 않는다(알파 0.03~0.10). */
.tabsec{{border:0;border-radius:12px;padding:14px 12px 18px;margin:0;
  background:linear-gradient(180deg,
    color-mix(in srgb, var(--tabc,#6b7480) 5%, transparent) 0,
    transparent 120px)}}
.tabsec-hd{{border-bottom:1px solid
  color-mix(in srgb, var(--tabc,#6b7480) 28%, transparent)}}
.tabsec-name::before{{content:"";display:inline-block;width:5px;height:5px;
  border-radius:50%;background:var(--tabc,#6b7480);margin-right:7px;
  vertical-align:3px}}
/* 소제목(sec-label)의 작은 꼬리표에도 같은 색을 옅게 — 탭마다 톤이 달라진다 */
.tabsec .sec-label small{{color:color-mix(in srgb, var(--tabc,#6b7480) 72%, #8b93a0)}}
...
/* 🔴 HO 지시 2026-09-12 — 다크 계열로 교체.
   [무엇이 문제였나] 배경이 rgba(255,255,255,.55) 흰색이라 다크 리포트에서
   이 칸만 하얗게 떠 보였다. v16 다크 전환 때 빠진 잔여 밝은 요소다.
   [기준] 바로 아래 .sc-guide(종목 카드)가 이미 다크다 — 그 톤에 맞춘다. */
.rd-guide{{background:#141922;border:1px solid #2a3446;
  border-radius:10px;padding:10px 11px;margin:8px 0 10px}}
.rd-guide-h{{margin:0 0 6px;font-size:12.5px;font-weight:800;color:#e8eaee}}
.rd-guide-b{{margin:0;font-size:11.5px;color:#c9ced6;line-height:1.75}}
.rd-guide-n{{margin:6px 0 0;font-size:10.5px;color:#7d848f;line-height:1.6}}
/* 🆕 2026-08-25 종목 카드 */
...
/* 🔴 2026-09-22 HO 지시 — 아래 두 칸(순매수 / 최근 5일 실탄)을 «같은 줄»로.
   두 칸 모두 위에서부터 채운다(가운데 정렬이면 내용 높이 차이만큼 어긋난다).
   숫자 칸은 옆 막대그래프 높이(64px)에 맞춰 세로 가운데 — 그래야
   «숫자 줄»과 «막대 줄»이 같은 높이에 선다. */
.sc4-c3,.sc4-c4{{justify-content:flex-start}}
/* ══ 📰 시황 탭 4단 ══ */
/* 🔴 HO 지시 2026-09-22 — 헤드라인 박스는 «다른 색». 탭의 첫 인상이다.
   보라-남색 바탕 + 오른쪽 위 호박색 빛. 세 줄마다 아이콘 칩 색이 다르다. */
.sh-top{{position:relative;overflow:hidden;
  background:radial-gradient(120% 90% at 100% 0%,rgba(255,176,64,.18),transparent 55%),
             linear-gradient(155deg,#231a3a 0%,#18203a 55%,#121a2a 100%);
  border:1px solid rgba(180,140,240,.35);border-radius:15px;padding:15px 14px 13px;
  margin:0 0 14px;box-shadow:0 6px 22px rgba(0,0,0,.35)}}
.sh-top::before{{content:'오늘 한눈에';display:block;font-size:10px;font-weight:800;
  letter-spacing:.08em;color:#c9b3f5;margin:0 0 10px}}
.sh-hl{{display:flex;gap:10px;align-items:flex-start}}
.sh-hi{{flex:none;width:30px;height:30px;border-radius:9px;display:inline-flex;
  align-items:center;justify-content:center;font-size:15px;margin-top:1px}}
.sh-hl.h0 .sh-hi{{background:rgba(255,176,64,.18);box-shadow:inset 0 0 0 1px rgba(255,176,64,.4)}}
.sh-hl.h1 .sh-hi{{background:rgba(94,234,212,.14);box-shadow:inset 0 0 0 1px rgba(94,234,212,.35)}}
.sh-hl.h2 .sh-hi{{background:rgba(255,120,120,.14);box-shadow:inset 0 0 0 1px rgba(255,120,120,.35)}}
.sh-hb{{flex:1;min-width:0}}
/* 코너마다 고유 색 — 왼쪽 띠 + 제목 색 */
.sh-box{{position:relative;border-left-width:3px !important}}
.sh-news{{border-left-color:#6ea8ff !important}}
.sh-news>.sh-h{{color:#9cc3ff}}
.sh-theme{{border-left-color:#ff9838 !important;
  background:linear-gradient(180deg,rgba(255,152,56,.05),transparent 40%),#101720}}
.sh-theme>.sh-h{{color:#ffb070}}
...
/* 이슈 태그 색 */
.sh-tag.t-semi{{color:#ffb070;background:rgba(255,152,56,.12)}}
.sh-tag.t-ind{{color:#c9b3f5;background:rgba(180,140,240,.13)}}
.sh-tag.t-pol{{color:#ffd76a;background:rgba(255,201,60,.12)}}
.sh-tag.t-sup{{color:#6fe3b8;background:rgba(62,207,154,.12)}}
.sh-tag.t-glo{{color:#9cc3ff;background:rgba(110,168,255,.13)}}
/* 사실 카드 구분 — 카드 사이를 조금 띄워 «한 덩어리 글»로 안 보이게 */
.sh-c{{padding:11px 0 !important}}
.sh-c+.sh-c{{border-top:1px dashed #1f2a38 !important}}
.sh-t{{margin:0;font-size:15px;font-weight:800;color:#eef1f5;line-height:1.45;letter-spacing:-.3px}}
.sh-hl{{padding:10px 0;border-top:1px solid rgba(255,255,255,.07)}}
.sh-hl:first-child{{border-top:0;padding-top:0}}
.sh-hl:last-child{{padding-bottom:0}}
.sh-hl.first .sh-t{{font-size:17.5px}}
.sh-s{{margin:5px 0 0;font-size:12px;color:#9aa3b1;line-height:1.6}}
.sh-ns{{display:flex;flex-wrap:wrap;gap:6px;margin:11px 0 0}}
.sh-n{{display:inline-flex;align-items:baseline;gap:5px;background:#0d141c;
  border:1px solid #1e2937;border-radius:8px;padding:5px 9px}}
.sh-n em{{font-style:normal;font-size:10px;color:#7d8695;font-weight:700}}
.sh-n b{{font-size:13px;font-weight:800;font-variant-numeric:tabular-nums}}
.sh-box{{background:#101720;border:1px solid #1e2937;border-radius:12px;
  padding:12px 13px 10px;margin:0 0 12px}}
.sh-h{{display:flex;align-items:baseline;gap:6px;margin:0 0 8px;font-size:14px;
  font-weight:800;color:#e2e7ee;list-style:none;cursor:default}}
.sh-h span{{margin-left:auto;font-size:9.5px;font-weight:600;color:#6f7784}}
.sh-c{{padding:10px 0;border-top:1px solid #172130}}
.sh-c:first-of-type{{border-top:0;padding-top:2px}}
.sh-tag{{display:inline-block;font-size:10.5px;font-weight:800;color:#8fd0e8;
...
.sh-tr{{display:flex;gap:8px;align-items:baseline;padding:6px 0;border-top:1px solid #172130;
  font-size:12px}}
.sh-tr:first-of-type{{border-top:0}}
.sh-tr span{{flex:none;width:78px;color:#7d8695;font-weight:700;font-size:11px}}
.sh-tr b{{color:#dfe4ea;font-weight:700;line-height:1.55}}
.sh-cm{{margin:9px 0 0;padding:10px 11px;border-radius:9px;background:rgba(255,152,56,.06);
  border:1px solid rgba(255,152,56,.2);font-size:13px;line-height:1.75;color:#e2e7ee}}
.sh-k{{padding:11px 0;border-top:1px solid #172130}}
.sh-k:first-of-type{{border-top:0;padding-top:2px}}
...
.sh-kw span{{display:block;font-size:10px;font-weight:800;color:#8fd0e8;margin:0 0 2px}}
.sh-ky{{margin:7px 0 0 28px;display:flex;flex-direction:column;gap:4px}}
.sh-ky p{{margin:0;font-size:12px;line-height:1.65;color:#aab3c0}}
.sh-ky i{{font-style:normal;font-size:9.5px;font-weight:800;border-radius:4px;padding:1px 6px;
  margin-right:6px}}
.sh-ky i.y{{color:#0b0e13;background:#3ecf9a}}
.sh-ky i.n{{color:#0b0e13;background:#8b93a0}}
.sh-fold summary{{cursor:pointer}}
.sh-fold summary::-webkit-details-marker{{display:none}}
.sh-empty{{margin:4px 0 0;font-size:11.5px;color:#7d8695;line-height:1.7}}
/* ══ 💰 수급 탭 5단 ══ */
.fv-box{{background:#101720;border:1px solid #1e2937;border-radius:12px;
  padding:12px 13px 11px;margin:0 0 12px}}
.fv-box,.fv-box *{{white-space:normal;word-break:keep-all;overflow-wrap:anywhere}}
.fv-h{{display:flex;align-items:baseline;gap:6px;flex-wrap:wrap;margin:0 0 9px;
  font-size:14px;font-weight:800;color:#e2e7ee}}
.fv-h span{{margin-left:auto;font-size:9.5px;font-weight:600;color:#6f7784}}
.fv-tabs{{display:flex;gap:5px;margin:0 0 9px}}
.fv-tb{{flex:1;padding:6px 0;font:inherit;font-size:11.5px;font-weight:800;color:#7d8695;
  background:#121a24;border:1px solid #1e2937;border-radius:7px;cursor:pointer}}
...
.fv-bar::after{{content:'';position:absolute;left:50%;top:-2px;bottom:-2px;width:1px;background:#2f3b4a}}
.fv-bar i{{position:absolute;top:0;bottom:0;border-radius:5px}}
...
.fv-pw{{padding:9px 0 11px;border-top:1px solid #172130}}
.fv-pw:first-of-type{{border-top:0;padding-top:2px}}
.fv-pt{{margin:0 0 22px;font-size:11.5px;color:#9aa3b1}}
.fv-pt b{{color:#dfe4ea}}
.fv-scale{{position:relative;height:6px;background:linear-gradient(90deg,#1b2530,#2a3646);
  border-radius:3px;margin:0 4px 26px}}
.fv-m{{position:absolute;top:-4px;width:14px;height:14px;margin-left:-7px;border-radius:50%;
  border:2px solid #0b0e13}}
.fv-m em{{position:absolute;left:50%;transform:translateX(-50%);white-space:nowrap !important;
  font-style:normal;font-size:9.5px;font-weight:800}}
.fv-m.s{{background:#5b9bff}} .fv-m.s em{{top:16px;color:#5b9bff}}
.fv-m.b{{background:#ff5a4e}} .fv-m.b em{{top:-17px;color:#ff8a72}}
.fv-m.now{{background:#ffc93c;width:16px;height:16px;margin-left:-8px;top:-5px}}
...
.fv-cap-h span{{float:right;color:#6f7784;font-weight:700}}
.fv-cap p{{margin:4px 0 0;font-size:12px;line-height:1.7;color:#c3cad4}}
.fv-wait{{margin:0;padding:12px 10px;border-radius:8px;background:#0d141c;
  border:1px dashed #2a3646;font-size:11.5px;color:#8b93a0;line-height:1.8}}
.fv-wait b{{color:#c3cad4}}
.fv-rt{{margin:0;font-size:15px;font-weight:800;color:#eef1f5}}
.fv-rs{{margin:4px 0 9px;font-size:12px;color:#9aa3b1;line-height:1.6}}
.fv-rg{{display:flex;flex-wrap:wrap;gap:6px}}
.fv-rg span{{background:#0d141c;border:1px solid #1e2937;border-radius:8px;padding:6px 9px;
  font-size:11px;color:#8b93a0}}
.fv-rg b{{color:#dfe4ea;margin:0 4px}}
.fv-rg i{{font-style:normal;font-weight:800}}
.fv-tr{{padding:8px 0;border-top:1px solid #172130}}
.fv-tr:first-of-type{{border-top:0;padding-top:0}}
.fv-tr span{{display:block;font-size:11px;font-weight:800;margin:0 0 3px}}
.fv-tr p{{margin:0;font-size:12.5px;line-height:1.75;color:#c3cad4}}
.fv-old{{margin:4px 0 0}}
.fv-old>summary{{cursor:pointer;font-size:11px;font-weight:700;color:#6f7784;
  padding:9px 12px;background:#0d141c;border:1px dashed #2a3646;border-radius:9px;list-style:none}}
.fv-old>summary::-webkit-details-marker{{display:none}}
/* ⚠️ 상위 규칙이 white-space:nowrap을 물려줘 글이 한 줄로 2,000px 늘어났다(실측).
   시황 카드 안은 전부 «보통 줄바꿈»으로 되돌린다. 링크 한 줄만 말줄임 유지. */
.sh-top,.sh-top *,.sh-box,.sh-box *{{white-space:normal;word-break:keep-all;
  overflow-wrap:anywhere}}
.sh-box .sh-ln{{white-space:nowrap;max-width:100%}}
/* 본문은 «보통 굵기». 굵게 물려받아 카드 전체가 제목처럼 보였다. */
.sh-p,.sh-d p,.sh-cm,.sh-kw,.sh-ky p{{font-weight:400 !important}}
/* 🔴 HO 지시 2026-09-22 — 시황 세 코너(무슨 일 / 이슈 해부 / 프로의 시선)
   라벨을 글 «옆»이 아니라 «위»로. 라벨 칸(≈60px)이 모바일 폭의 1/6을
   먹어 글이 좁은 기둥처럼 길게 늘어졌다. 라벨을 위로 올리면 글이
   카드 폭을 다 쓴다 — 같은 글이 줄 수는 약 30% 줄어든다. */
.i9,.iss,.si-item{{display:block !important;padding:10px 0 !important}}
.i9-t,.itag,.si-lens{{display:inline-flex !important;margin:0 0 5px !important;
  flex-direction:row !important;gap:5px !important;align-items:baseline !important}}
.i9>span:not(.i9-t),.iss-text,.si-item>span:not(.si-lens){{display:block;width:100%}}
.sc3-body{{min-height:64px;align-content:center;width:100%}}
...
/* ── v17 테마 3코너 ── */{THEME_V17_CSS}{LEADER_CSS}</style>
</head>
<body>
<!-- 🔴🔴 2026-09-17 버그 수정 — 테마명을 눌러도 종목이 안 펼쳐졌다.
     [원인] ztog() 함수 «정의»가 build_sector_scoreboard() 안에 들어 있었다.
       HO 지시로 「섹터 성적표」 코너를 숨기자, 그 코너가 들고 있던
       스크립트까지 같이 사라졌다. 테마 레이더·다가오는 테마·섹터×테마가
       전부 이 함수를 쓰는데, 정의만 남의 코너에 얹혀 있었던 것이다.
     [증상] 콘솔에 «ztog is not defined». 클릭해도 아무 일도 안 일어난다.
       ⚠️ 화면은 멀쩡해 보인다 — 눌러 보기 전에는 아무도 모른다.
     [고침] 페이지가 시작하자마자 «한 번» 정의한다. 어느 코너를 숨기든
       살아남는다.
     ⚠️ 교훈: 여러 코너가 같이 쓰는 코드는 «특정 코너 안»에 두지 않는다.
       코너는 언제든 가려질 수 있다(원칙3의 부작용). -->
{ZTOG_JS}
<div class="rp">
  <div class="top-bar">
