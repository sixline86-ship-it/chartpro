...
#    시점에 preview 사본을 repo(실제 전달 원본) 위에 그대로 덮어써서
#    이 임시 고정값이 같이 딸려 들어갔다. 그 뒤로 전달된 build_html.py가
#    전부 "8월 31일"에 멈춘 채였다 — 오늘(9/1) 발행이 매번
#    `report_20260901.html`을 못 찾고 실패한 진짜 원인이 이거였다.
#    from_cron·cron-job.org 쪽 문제가 아니었다(그쪽은 22번 이상 잘못
#    의심받았다). ⚠️ 앞으로 미리보기 작업은 **반드시 /home/claude/preview
#    사본에서만** 날짜를 고정하고, 그 사본을 절대 repo로 되돌려 복사하지
#    않는다 — 이 사고의 재발 방지책이다.
# ── 파일 보관 위치 ──
