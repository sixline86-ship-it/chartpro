...
BRIEF_NEWS_DAYS = 5     # 🆕 「내 종목 브리핑」이 뒤지는 기간(거래일)
...
def build_core_accum(매집):
    ...
    """
    중기 = (매집 or {}).get("중기종목") or []
    if not 중기:
        return ""
    기간 = (매집 or {}).get("중기기간", 20)
...
def build_my_stocks(data):
    ...
    try:
        ...
        for _n in _snews:
            _snews[_n] = list(reversed(_snews[_n]))[:5]
    except Exception as e:
...
(function(){
 ...
 function _josa(w,pair){
  ...
  if(ch>=0xAC00&&ch<=0xD7A3) return ((ch-0xAC00)%28)?pair[0]:pair[1];
  var c=t[t.length-1];
  if(c>='0'&&c<='9') return ('0136780'.indexOf(c)>=0)?pair[0]:pair[1];
  return pair[1];
 }
 ...
 function render(){
  ...
  my.forEach(function(nm){
   ...
   if(c&&!c.short){exs.push(c.ex);
    ...
    right='<div style="text-align:right;flex:none;width:74px">'+
     '<div style="font-size:13.5px;font-weight:800;color:'+_주색+'">'+_주+'</div>'+
     '<div style="font-size:9.5px;color:#7d848f">'+_보+'</div>'+
     /* 🆕 2026-08-25 HO 지시 — 당일 탭의 '시장 상회/하회' 글자 제거.
        [WHY] 바로 위 줄이 이미 '시장 대비 -5.6%p'라고 말한다.
              부호만 봐도 아는 걸 한 줄 더 쓰면 칸만 좁아진다. */
     (c.tot<=1 ? '' : '<div style="font-size:9px;color:#6f7784">'+
        c.win+'/'+c.tot+'승</div>')+
     '</div>';
 ...
 function drawBrief(my){
  ...
  my.forEach(function(nm){
   ...
   hits.slice(0,4).forEach(function(h){
    var n=h.n;
    /* 🆕 2026-08-26 — 날짜를 반드시 붙인다. 5거래일치를 섞어 보여주므로
       안 붙이면 사흘 전 기사를 오늘 일로 오해한다. */
    ...
    items+='<div style="display:flex;gap:6px;margin-top:4px"><span style="flex:none">📰</span>'+
     '<div>'+날+'<a href="'+n.u+'" target="_blank" style="font-size:11px;color:#8fb4ee;'+
   ...
   if(n1>4) items+='<p style="margin:4px 0 0;font-size:10px;color:#6f7784">'+
     '외 '+(n1-4)+'건 더 있습니다 · 최근 '+(window.CP_BRIEF_DAYS||5)+'거래일</p>';
    ...
    if(hits.length){
     ...
     if(window.CP_SECT_TODAY){
      ...
      if(bb2&&Math.abs(bb2.v)>=1.0)
       {볼것.push('소속 구역 <b>'+bb2.z+'</b>'+_josa(bb2.z,'이가')+' '+fmt(bb2.v)+'% 움직였으니, 이 종목도 그 흐름을 따라갔을 가능성이 큽니다');
        _구역말함=true;}
     }
     ...
     if(window.CP_MARKET){
      ...
      ['외국인','기관'].forEach(function(who){
       ...
       if(o&&o.연속>=3&&볼것.length<2)
        볼것.push('시장 전체로는 <b>'+who+'이 '+o.연속+'일 연속 '+o.방향+'</b> 중입니다');
      });
     }
   ...
   out+='<div style="padding:11px 0;border-bottom:1px solid #1b212c">'+
    ...
    '<b style="color:#e0c060">오늘 분석</b> — '+원인+분석+'</p></div>'+
    '<div id="'+_bid+'" style="display:none"></div></div>';
  });
  host.innerHTML=out;
 ...
 function _gauge(pct, 기준){     /* 부채비율 게이지 */
  var w=110,h=10, x=Math.max(0,Math.min(1,pct/(기준*2)))*w;
  ...
  return '<svg viewBox="0 0 '+w+' '+h+'" style="width:'+w+'px;height:'+h+'px;vertical-align:middle">'+
   '<rect x="0" y="3" width="'+w+'" height="4" rx="2" fill="#232a36"/>'+
   '<line x1="'+(w/2)+'" y1="0" x2="'+(w/2)+'" y2="'+h+'" stroke="#6f7784" stroke-width="1"/>'+
   '<circle cx="'+x.toFixed(1)+'" cy="5" r="4" fill="'+c+'"/></svg>';
 }
 function _why(nm){
 ...
 function _fin(nm, 상세){
  ...
  if(ys.length>=2 && 첫.매출 && 끝.매출){
   ...
   증=' <span style="font-size:10px;color:#8b93a0">'+ys[0]+'→'+ys[ys.length-1]+' '+fmt(g)+'%</span>';
  }
  var 부채='';
  if(끝.부채&&끝.자본){
   var r=끝.부채/끝.자본*100;
...
def build_sector_ladder():
    ...
    for nm, col, pts, 끝y in 코스:
        ...
        _줄.append(
            ...
            f'{nm}</p><p style="margin:0 0 5px">{_요약}</p><div>{칩}</div></div>')
    if _줄:
