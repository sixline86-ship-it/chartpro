...
SCRIPT_VERSION = "v2026.09.17-v19"   # ⬅ 다른 파일과 항상 같아야 한다.
...
# 사유 코드 → 사람이 읽는 안내
안내표 = {
    # 🔴 2026-09-17 문구 교체 — 옛 문구는 «수급이 없다»를 휴장 근거로 들었다.
    #    실제 사고(9/17): 네이버가 수급 페이지를 폐지해 수급만 비었는데
    #    거래대금 정상인 멀쩡한 거래일이 「휴장」으로 나갔다.
    #    이제 휴장 판정은 «거래대금»이 근거이므로 문구도 그렇게 바꾼다.
    "holiday": (
        "🚨 오늘 휴장으로 판단됩니다",
        "오늘 거래대금이 0이거나 정상 범위를 크게 밑돕니다.\n"
        "임시공휴일이거나 임시 휴장일 가능성이 큽니다.\n\n"
        "✅ 조치: 아무것도 안 했습니다. 과금도 발송도 없었습니다.\n"
        "   실제로 장이 열린 날이었다면 수집원(네이버·KRX) 장애를 의심하세요."
    ),
    # 🆕 2026-09-17 신설 — «수집 실패»는 휴장과 조치가 정반대다.
    #    휴장은 아무것도 안 하면 되지만, 이건 사람이 손을 대야 한다.
    "flow_fail": (
        "🔧 수급만 못 읽었습니다 (휴장 아님)",
        "거래대금은 정상인데 오늘 수급(외국인·기관)만 비었습니다.\n"
        "장은 열렸습니다 — 우리 쪽 수집원 문제입니다.\n\n"
        "✅ 조치: 발행은 그대로 진행했습니다.\n"
        "   리포트에서 수급 관련 칸만 «미확보»로 비어 있습니다.\n\n"
        "🔍 확인할 것\n"
        " · 네이버 수급 페이지가 폐지·이전되지 않았는지\n"
        "   (2026-09-17에 investorDealTrendDay가 실제로 폐지됨)\n"
        " · 새 경로를 잡았다면 collect_data.py의 수급() 함수를 교체"
    ),
    "no_interp": (
        "⚠️ 해석글이 없어 발송을 멈췄습니다",
