...
SCRIPT_VERSION = "v2026.08.29-a20"   # ⬅ 다른 파일과 항상 같아야 한다.
...
안내표 = {
    ...
    # 🆕 2026-08-27 (2차) 실측 사고 — GitHub 무료 예약이 10시간 넘게 밀려
    #   16:50 예약이 다음 날 새벽 3시대에 실행됐다. 그 상태로는 코드가
    #   "오늘"을 착각해 열리지도 않은 시장 데이터를 잠정으로 만든다.
    #   daily.yml이 이 경우 텔레그램만 건너뛰지만, 형에게는 알려야 한다.
    "abnormal_time": (
        "🔴 예약이 심하게 밀려 새벽에 실행됐습니다",
        "GitHub 무료 예약이 밀려 정오 이전(보통 새벽)에 실행됐습니다.\n"
        "이 시각엔 코드가 '오늘'을 이미 다음 날로 착각해, 아직 열리지도\n"
        "않은 시장을 대상으로 잠정 데이터를 만듭니다.\n\n"
        "✅ 조치: 텔레그램은 자동으로 건너뛰었습니다(리포트 페이지는 그대로\n"
        "   발행됨). 다음 정상 시각대(16:50 이후) 회차가 다시 시도합니다.\n"
        "   이 알림이 자주 오면 GitHub Actions 예약 자체가 반복해서 밀리고\n"
        "   있다는 뜻이니, 예약 시간을 더 당기는 걸 고려하세요."
    ),
    "inherited": (
        "🔁 전날 해석글로 발행했습니다",
