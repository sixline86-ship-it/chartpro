...
# 🔴 2026-09-03 실전 규모(2,908개) 점검에서 발견한 사고 — 회사명 환각.
#    [발견] "한전기술"의 설명이 "한국수력원자력은..."으로, "에코프로비엠"이
#    "포스코퓨처엠(또는 유사 회사)은..."으로 시작하는 등, 실제 존재하지
#    않는 근거로 **완전히 다른 회사 이름**을 지어낸 사례가 199개(체크
#    대상의 10.8%) 발견됐다.
#    [원인] 사업보고서 원문은 자기 회사를 "당사"라고만 쓰고 실제
#    회사명이 안 보이는 경우가 흔한데, 프롬프트가 "[회사명]은~"으로
#    무조건 시작하라고 강제하니 모델이 사업 내용만 보고 "이런 사업
#    하는 유명한 회사면 OO겠지"라고 잘못 추측했다. 규칙3(계열사명·
#    제품명은 원문에 있는 것만)은 있었는데, 정작 "자기 회사 이름"에
#    대한 규칙이 빠져 있었다.
#    [조치] ①위 199개를 biz_description.json에서 제거해 자동 재파싱
#    대상으로 돌림. ②아래 SYSTEM_PROMPT에 규칙 3-1 추가(원문에 회사명
#    없으면 "이 회사는"으로 시작, 얼버무리는 표현 금지).
...
3-1. ⚠️ 가장 중요 — **회사 이름 자체를 절대 추측하지 마라.** 사업보고서
   원문은 자기 회사를 "당사"라고만 쓰고 실제 회사명이 안 보이는 경우가
   흔하다. 이때 사업 내용(예: "원자력발전소 설계")만 보고 "이런 사업을
   하는 유명한 회사면 OO겠지"라고 절대 추측해서 이름을 채우지 마라 —
   전혀 다른 회사 이름을 지어내는 가장 위험한 실수다. 원문에 정확한
   회사명이 안 보이면 첫 문장을 "이 회사는 ~"로 시작해라. 확신이 없는
   회사명을 "(또는 유사 회사)"처럼 얼버무려 쓰는 것도 금지 — 얼버무릴
   바에 "이 회사는"으로 시작해라.
